...
description: Use when modifying the Minova GitHub Pages, Cloudflare Worker, or D1 application, especially UI, permissions, persisted state, Hybrid EPC Design, Product Master, suppliers, inventory, certifications, quote/PDF, saved quotes, or sync behavior.
---
...
Work as a careful maintainer of an operations-focused business tool. Read `AGENTS.md`/`agents.md` first, then verify the exact current code path with `rg` before editing. Prefer small, compatible changes over broad rewrites because `index.html` owns most UI and browser behavior, `epc-design-engine.mjs` owns Hybrid EPC calculations, and `worker/` plus `auth/` own D1-backed data and permissions.
...
- For Hybrid EPC, check both `epc-design-engine.mjs` and `epc-design-engine.global.js`; the first is testable source, the second is the browser-loaded build.
- For state changes, check `applyStateFromData()`, `saveToLocal()`, `window.getMinovaBusinessStateSnapshot()`, `window.buildUpdatedHtml()`, `window.__minovaSync` `getLocalState`, `github-sync/merge.js`, `minova-data/state.json`, and localStorage fallback keys.
...
- The top-level tab is `epcdesign`. Quick design and detailed engineering are permissioned separately through `epcDesign` and `epcDesignEngineering`.
- `epc-design-engine.mjs` covers sizing, solar resource, diesel replacement, EMS Flow, Device Work, Battery Control, PV Simulator, topology, SLD, electrical architecture, BOQ, risks, and report data.
- `index.html` owns EPC DOM panels, input capture, localStorage/D1 writes, report buttons, XLSX export, and fixed-page PDF rendering.
- EPC state uses `epcDesignProjects`, `epcDesignDefaults`, `minova_epc_design_projects_v1`, `minova_epc_design_defaults_v1`, `epc_design_project`, and `epc_design_defaults`.
- EPC panels include schemes, formula trace, BOQ, risks, topology, electrical, EMS Flow, Device Work, Battery Control, PV Simulator, and reports.
- Open High risk items intentionally block EPC Reports PDF until the existing acknowledgement UI saves a mitigation reason and typed signature.
- Do not replace the EPC engine or topology model for UI-only requests; deepen the current workflow unless the user explicitly asks for a redesign.

## Product Master And Supply Chain

- Products remain canonical in `products`; new `masterData`, `technicalSpecs`, `sourcing`, base-price, compatibility, and certification fields must coexist with legacy `vendor`, `supplierCode`, `category`, and `spec`.
...
- `channelPartners` backs Authorized Distributor / Dealer / Partner relationships, and product `sourcing.channelPartnerId` may reference it. Preserve supplier and channel delete protections.
- `compatibilityRules`, `certificationRequirementsCatalog`, `productCertificationEvidence`, and `productMasterDetailTemplates` are persisted state and D1-backed business domains.
- Product import/export must keep legacy Chinese headers while covering sourcing, compatibility, certification, and Channel Partners sheets.
...
- Non-stock pricing lives in `nonStockPricingStrategies`; Authorized Distributor default tax/fee behavior is tested.
...
- For shared frontend, D1, permissions, Product Master, EPC persistence, or business-state changes, run the complete root test suite:
...
- For Hybrid EPC changes, run targeted EPC coverage as appropriate:
...
- For Product Master / Engineering Workspace changes, start with:
...
- For UI, PDF, saved quotes, attachments, Site Overview, or EPC Reports changes, start a local static server and verify the affected workflow in a browser:

```bash
python3 -m http.server 8080
```

- Restricted tabs include quotation, quote settings, product list, EPC design, engineering, price list, inventory, and transport. Use `file://` mode or an authenticated/unlocked HTTP session when checking hidden surfaces.
- If one browser verification route fails repeatedly from file URL, sandbox, cache, or process-permission issues, stop after three tries and report the skipped route plus the fallback used.
...
- Do not add a top-level tab or workspace without updating `auth/permission-core.mjs`, admin permission UI read/write, Worker resource mapping, and round-trip tests.
...
- Do not treat EPC report download blocks as renderer bugs before clearing the existing High risk acknowledgement gate.

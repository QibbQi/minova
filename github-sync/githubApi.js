...
export function createGitHubApi({ tokenProvider, limiter }) {
  ...
  async function requestText(url, init, attempt = 0) {
    await limiter.wait()
    const token = await tokenProvider.getToken()
    const headers = new Headers(init?.headers || {})
    headers.set('Accept', 'application/vnd.github+json')
    ...
    if (res.status === 429 || res.status === 500 || res.status === 502 || res.status === 503 || res.status === 504) {
      if (attempt < 4) {
        ...
        const backoff = retryAfter ? parseInt(retryAfter, 10) * 1000 : (2 ** attempt) * 800
        await sleep(backoff)
        return requestText(url, init, attempt + 1)
      }
    }

    if (res.status === 403) {
      const msg = String(await res.text())
      const isRateLimited = msg.toLowerCase().includes('rate limit exceeded')
      if ((isRateLimited || remaining === '0') && resetMs > 0 && attempt < 2) {
        await sleep(Math.min(resetMs, 5 * 60_000))
        return requestText(url, init, attempt + 1)
      }
  ...
  return {
    ...
    fetchText: (url, init) => requestText(url, init || { method: 'GET' })
  }
}

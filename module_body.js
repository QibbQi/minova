...
`;
        ...
        function renderProductCertificationMatrix() {
            ...
            list.innerHTML = rows.length ? rows.map(row => {
                ...
                return `<div class="rounded-2xl border ${checked ? 'border-purple-200 bg-purple-50/30' : 'border-slate-200 bg-white'} p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 xl:grid-cols-6 gap-2 mt-3">
                        <label class="block">
        ...
        window.setCertificationClassSelection = (classId) => {
            const id = String(classId || 'class_a').trim();
        ...
        function productMatchesCertificationClass(product, profile) {
            const groupLabel = getProductTypeGroup(product).label;
            return (profile.productGroups || []).some(group => {
                if (group === 'Inverter / PCS') return groupLabel === 'Inverter';
                if (group === 'ESS') return groupLabel.includes('ESS');
                if (group === 'BOS') return groupLabel.includes('BOS');
                return group.includes(groupLabel) || groupLabel.includes(group);
            });
        }
        function productPassesCertificationClassFilters(product) {
            const outdoor = document.getElementById('cert-class-outdoor-filter')?.value || 'all';
            const gridDischarge = document.getElementById('cert-class-grid-discharge-filter')?.value || 'all';
            const capacity = parseFloat(document.getElementById('cert-class-capacity-filter')?.value || '');
            const text = flattenProductMasterValues([product.masterData, product.technicalSpecs, product.spec, product.scenario]).join(' ').toLowerCase();
            if (outdoor === 'outdoor' && !text.includes('outdoor') && !text.includes('ip')) return false;
            if (outdoor === 'indoor' && text.includes('outdoor')) return false;
            if (gridDischarge === 'yes' && !text.includes('grid') && !text.includes('pcs') && !text.includes('hybrid')) return false;
            if (gridDischarge === 'no' && text.includes('grid discharge')) return false;
            if (Number.isFinite(capacity) && capacity > 0) {
                const summary = [productMasterTechnicalSummary(product), productMasterEfficiencyCapacity(product), product.spec].join(' ');
                const nums = summary.match(/\d+(?:\.\d+)?/g)?.map(Number) || [];
                if (nums.length && Math.max(...nums) < capacity * 0.5) return false;
            }
            return true;
        }
        function certificationProductCardHtml(product, profile) {
            const status = getProductCertificationV4Status(product, profile.id);
            const compat = getProductCompatibilitySummary(product);
            return `<div class="rounded-xl border border-slate-200 bg-white px-3 py-3">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-xs font-black text-slate-800 truncate">${htmlSafe(product.id || '')} · ${htmlSafe(productListDisplayText(product.name))}</div>
                        <div class="text-[10px] text-slate-400 mt-1 truncate">${htmlSafe(productMasterTechnicalSummary(product) || getProductDisplaySpec(product) || '-')}</div>
                    </div>
                    ${productMasterStatusPill(status.status, status.tone)}
                </div>
                <div class="mt-2 flex flex-wrap gap-1.5">
                    ${productMasterStatusPill(`Compatibility: ${compat.label}`, compat.status === 'Approved' ? 'green' : (compat.status === 'Blocked' ? 'red' : 'slate'))}
                    ${productMasterStatusPill(getProductSourcing(product).sourceType || 'Unknown', 'slate')}
                </div>
            </div>`;
        }
        function renderCertificationClassProductStack(profile) {
            const stack = document.getElementById('cert-class-product-stack');
            const summary = document.getElementById('cert-class-stack-summary');
            if (!stack) return;
            const groups = [
                ['PV Module', p => getProductTypeGroup(p).id === 'pv'],
                ['Inverter / PCS', p => getProductTypeGroup(p).id === 'inverter'],
                ['Battery', p => getProductTypeGroup(p).id === 'battery'],
                ['ESS', p => getProductTypeGroup(p).id === 'ess'],
                ['BOS', p => getProductTypeGroup(p).id === 'bos']
            ].filter(([label]) => (profile.productGroups || []).includes(label) || (label === 'Inverter / PCS' && (profile.productGroups || []).includes('Inverter / PCS')));
            let total = 0;
            stack.innerHTML = groups.map(([label, predicate]) => {
                const rows = products.filter(p => predicate(p) && productMatchesCertificationClass(p, profile) && productPassesCertificationClassFilters(p)).slice(0, 8);
                total += rows.length;
                return `<div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-3">
                    <div class="flex items-center justify-between mb-2">
                        <h4 class="text-xs font-black text-slate-700 uppercase tracking-widest">${htmlSafe(label)}</h4>
                        <span class="text-[10px] font-black text-slate-400">${rows.length}</span>
                    </div>
                    <div class="space-y-2">${rows.length ? rows.map(p => certificationProductCardHtml(p, profile)).join('') : '<div class="rounded-xl border border-dashed border-slate-200 bg-white p-3 text-xs text-slate-400">No matching products.</div>'}</div>
                </div>`;
            }).join('');
            if (summary) summary.textContent = `${total} products`;
        }
        function renderCertificationClassChecklist(profile) {
            ...
            if (!box) return;
            const items = getCertificationChecklistForClass(profile.id);
            if (summary) summary.textContent = `${items.length} items`;
            box.innerHTML = items.map(item => {
                const status = item.notApplicable ? 'N/A' : (item.level === 'Mandatory' ? 'Needs Review' : item.level);
                return `<div class="rounded-xl border border-slate-200 bg-white p-3">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <div class="text-xs font-black text-slate-800">${htmlSafe(item.code)}</div>
                            <div class="text-[10px] text-slate-500 mt-1">${htmlSafe(item.documentType)}</div>
                        </div>
                        ${productMasterStatusPill(status, certificationLevelTone(item.level, status))}
                    </div>
                    <div class="mt-2 text-[10px] text-slate-400 leading-relaxed">${htmlSafe(item.notApplicable ? item.reason : item.applicabilityCondition || item.requiredEvidence || '-')}</div>
                </div>`;
            }).join('');
        }
        ...
        function renderCertificationClassWorkspace() {
            ...
            renderCertificationClassProductStack(profile);

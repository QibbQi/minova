...
`;
        ...
        function renderProductMasterDetailBulkEditorControl(product = {}, template = {}, fieldKey = '') {
            ...
            if (!disabled && choices.length) {
                return `<select ${commonAttrs} class="w-full min-w-[180px] border border-slate-200 rounded-lg px-2 py-2 text-xs outline-none focus:border-purple-500 bg-white">
                    <option value="" ${value ? '' : 'selected'}></option>
                    ${choices.map(choice => `<option value="${htmlSafe(choice)}" ${choice === value ? 'selected' : ''}>${htmlSafe(choice)}</option>`).join('')}
                </select>`;
            }
            return `<input ${commonAttrs} list="${htmlSafe(datalistId)}" value="${htmlSafe(value)}" ${disabled ? 'disabled' : ''} class="w-full min-w-[180px] border border-slate-200 rounded-lg px-2 py-2 text-xs outline-none focus:border-purple-500 ${disabled ? 'bg-slate-50 text-slate-400' : 'bg-white'}">`;
        }
        ...
        function previewEngineeringProductMasterBulkSave() {
            ...
            const template = currentProductMasterDetailTemplate();
            const changes = [];
            document.querySelectorAll('#engineering-detail-template-bulk-list input[data-engineering-detail-product], #engineering-detail-template-bulk-list select[data-engineering-detail-product]').forEach(input => {
        ...
        function addEngineeringProductToQuote(productId) {
            if (!canAddEngineeringProductToQuote()) return alert('No quotation edit permission.');
            const product = products.find(item => String(item.id || '') === String(productId || ''));
            if (!product) return alert('Product not found.');
            const defaults = getEngineeringQuoteDefaults();
            const addFromPriceList = (priceType = defaults.priceType) => window.pickPriceListProduct?.(product.id, priceType);

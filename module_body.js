...
`;
        ...
        function normalizeCertificationRequirements(value, product = {}) {
            if (value && typeof value === 'object') {
                ...
                return {
                    countries: Array.isArray(value.countries) ? value.countries.map(v => String(v || '').trim()).filter(Boolean) : [],
            ...
            return { countries: [], standards: [], source: 'matrix', updatedAt: '' };
        }
        ...
        try {
            ...
            if (raw) {
                ...
                if (embedded?.data) {
                    ...
                    certificationStandardsCatalog = Array.isArray(embedded.data.certificationStandardsCatalog) ? embedded.data.certificationStandardsCatalog : [];
                    certificationClassProfiles = Array.isArray(embedded.data.certificationClassProfiles) ? embedded.data.certificationClassProfiles : [];
                    productCertificationEvidence = Array.isArray(embedded.data.productCertificationEvidence) ? embedded.data.productCertificationEvidence : [];
                    try { localStorage.setItem('minova_compatibility_rules_v1', JSON.stringify(compatibilityRules)); } catch (e) {}
                    try {
                        localStorage.setItem('minova_certification_standards_v1', JSON.stringify(certificationStandardsCatalog));
                        localStorage.setItem('minova_certification_class_profiles_v1', JSON.stringify(certificationClassProfiles));
        ...
        try {
            const raw = localStorage.getItem('minova_certification_standards_v1');
            if (raw) certificationStandardsCatalog = Array.isArray(JSON.parse(raw)) ? JSON.parse(raw) : certificationStandardsCatalog;
        } catch (e) {}
        try {
            const raw = localStorage.getItem('minova_certification_class_profiles_v1');
            if (raw) certificationClassProfiles = Array.isArray(JSON.parse(raw)) ? JSON.parse(raw) : certificationClassProfiles;
        } catch (e) {}
        try {
            const raw = localStorage.getItem('minova_product_certification_evidence_v1');
            if (raw) productCertificationEvidence = Array.isArray(JSON.parse(raw)) ? JSON.parse(raw) : productCertificationEvidence;
        } catch (e) {}
        try {
        ...
        function applyStateFromData(data, stampMs = 0) {
            ...
            certificationStandardsCatalog = normalizeCertificationStandardsCatalog(data?.certificationStandardsCatalog);
            certificationClassProfiles = normalizeCertificationClassProfiles(data?.certificationClassProfiles);
        ...
        window.getMinovaBusinessStateSnapshot = () => ({
            ...
            data: {
                ...
                transportRecords,
                compatibilityRules,
                certificationStandardsCatalog,
                certificationClassProfiles,
        ...
        const PRODUCT_ROLE_VIEWS = [
            ...
            { id: 'engineering', label: 'Engineering / Technical', columns: ['id', 'name', 'category', 'classFit', 'technicalSummary', 'compatibilityStatus', 'certificationV4Status', 'missingEvidence', 'technicalFit', 'actions'] },
        ...
        const CERTIFICATION_LEVELS = ['Mandatory', 'Utility Preferred', 'International Finance Preferred', 'Optional'];
        const CERTIFICATION_EVIDENCE_STATUS = ['Ready', 'Gap', 'Conditional', 'N/A', 'Expired', 'Needs Review'];
        const CERTIFICATION_STANDARDS_CATALOG_DEFAULTS = [
            { matrixRecordId: 'PV-001', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 61215 series', requirementLevel: 'Mandatory', applicabilityCondition: 'Grid-connected PV module; select the technology-specific part and current edition.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/service/photovoltaic-pv-module/', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-002', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 61730-1', requirementLevel: 'Mandatory', applicabilityCondition: 'PV module construction safety qualification.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/service/photovoltaic-pv-module/', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-003', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 61730-2', requirementLevel: 'Mandatory', applicabilityCondition: 'PV module safety testing.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/service/photovoltaic-pv-module/', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-004', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'MS IEC 61215 / MS IEC 61730', requirementLevel: 'Mandatory', applicabilityCondition: 'Only when Malaysian Standard, government procurement or tender documentation explicitly requires MS adoption.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/service/photovoltaic-pv-module/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-005', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 60904 series', requirementLevel: 'Utility Preferred', applicabilityCondition: 'PV device measurement and performance verification for utility/LSS review.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-006', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 61853 series', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Energy rating and performance over irradiance/temperature range.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-007', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 61701', requirementLevel: 'Optional', applicabilityCondition: 'Salt-mist exposure; condition-specific for coastal or marine sites.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-008', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 62716', requirementLevel: 'Optional', applicabilityCondition: 'Ammonia corrosion; condition-specific for agricultural/industrial sites.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-009', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC TS 62804 series', requirementLevel: 'Utility Preferred', applicabilityCondition: 'PID resistance, especially for 1500 V and humid/high-temperature projects.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-010', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 62782', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Dynamic mechanical load; utility-scale and tracker/high-wind applications.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-011', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 62941', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'PV module manufacturing quality system and ongoing product consistency.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-012', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC TS 62915', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Retesting and type approval after design, BOM or factory changes.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-013', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC 62759-1', requirementLevel: 'Optional', applicabilityCondition: 'Transportation packaging qualification for PV modules.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-014', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'IEC TS 63126', requirementLevel: 'Optional', applicabilityCondition: 'High-temperature module qualification for hot roofs or poorly ventilated installation.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'PV-015', matrixSheet: 'PV_Module_Cert', matrixType: 'PV Module', productCategory: 'PV Module', standardRequirement: 'SIRIM Product Certification', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Tender/programme-specific; do not treat as universally mandatory without an official product-specific order.', evidenceType: 'Product Listing / License', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/service/photovoltaic-pv-module/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-001', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 62109-1', requirementLevel: 'Mandatory', applicabilityCondition: 'Grid-connected PV inverter safety — general requirements.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://efit.seda.gov.my/?id=3588&omaneg=00010100000001010101000100001000000010100001000110', projectApplicability: 'Grid-connected only', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-002', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 62109-2', requirementLevel: 'Mandatory', applicabilityCondition: 'PV inverter safety — particular requirements.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://efit.seda.gov.my/?id=3588&omaneg=00010100000001010101000100001000000010100001000110', projectApplicability: 'Grid-connected only', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-003', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 61727', requirementLevel: 'Mandatory', applicabilityCondition: 'Grid interface characteristics for applicable Malaysian PV interconnection programme/project.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://efit.seda.gov.my/?id=3588&omaneg=00010100000001010101000100001000000010100001000110', projectApplicability: 'Grid-connected only', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-004', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 62116', requirementLevel: 'Mandatory', applicabilityCondition: 'Anti-islanding test for grid-connected inverter/PCS; not applicable to standalone off-grid operation.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://efit.seda.gov.my/?id=3588&omaneg=00010100000001010101000100001000000010100001000110', projectApplicability: 'Grid-connected only', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-005', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 61683', requirementLevel: 'Mandatory', applicabilityCondition: 'Efficiency measurement report for applicable SEDA/TNB equipment approval or project submission; not a safety certificate.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://efit.seda.gov.my/?id=3588&omaneg=00010100000001010101000100001000000010100001000110', projectApplicability: 'Grid-connected only', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-006', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'BESS PCS', standardRequirement: 'IEC 62477-1', requirementLevel: 'Mandatory', applicabilityCondition: 'BESS power conversion system safety evaluation under ST BESS Guidelines.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-007', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 61000-3-2 / IEC 61000-3-12', requirementLevel: 'Utility Preferred', applicabilityCondition: 'LV harmonic-current limits; choose the applicable part by rated current.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-008', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 61000-3-3 / IEC 61000-3-11', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Voltage fluctuation and flicker; choose the applicable part.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-009', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'BESS PCS', standardRequirement: 'IEC 61000-6-2', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Industrial-environment EMC immunity.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'C&I', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-010', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'BESS PCS', standardRequirement: 'IEC 61000-6-4', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Industrial-environment EMC emission.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'C&I', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-011', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'EN 50530', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'MPPT and conversion-efficiency performance assessment.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-012', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEC 62891', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'IEC MPPT efficiency measurement.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-013', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'IEEE 1547 / IEEE 1547.1', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Only when specified by TNB, utility tender, microgrid or project grid-code requirements.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.mytnb.com.my/renewable-energy/solar-accelerated-transition-action-programme', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-014', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'BESS PCS', standardRequirement: 'IEC 61850 communication', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Utility PCS/SCADA/plant-controller integration; verify implemented logical nodes and files.', evidenceType: 'Functional Test', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-015', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'BESS PCS', standardRequirement: 'LVRT / HVRT / FRT', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Utility or MV/HV projects; test against project Grid Code settings.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.mytnb.com.my/renewable-energy/solar-accelerated-transition-action-programme', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-016', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'P-Q / Volt-VAR / Volt-Watt / Frequency-Watt', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Advanced grid-support functions where required by current utility connection documents.', evidenceType: 'Functional Test', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.mytnb.com.my/renewable-energy/solar-accelerated-transition-action-programme', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-017', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'Hybrid Inverter', standardRequirement: 'Export Limitation / Zero Export', requirementLevel: 'Mandatory', applicabilityCondition: 'Only where SELCO, zero-export or project approval requires export limitation.', evidenceType: 'Functional Test', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-018', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'BESS PCS', standardRequirement: 'IEC 60529 / IP rating', requirementLevel: 'Mandatory', applicabilityCondition: 'Outdoor BESS PCS/enclosure: ST requires IP54 or higher depending on site conditions.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'INV-019', matrixSheet: 'Inverter_PCS_Cert', matrixType: 'Inverter / PCS', productCategory: 'PV Inverter', standardRequirement: 'SIRIM Product Certification / Listing', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Tender/programme-specific; verify exact model and active license/listing.', evidenceType: 'Product Listing / License', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/service/photovoltaic-pv-module/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-001', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'IEC 62619', requirementLevel: 'Mandatory', applicabilityCondition: 'Battery pack/system and BMS technical evaluation for industrial/stationary lithium battery applications.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-002', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'IEC 63056', requirementLevel: 'Mandatory', applicabilityCondition: 'Battery pack/system and BMS technical evaluation for electrical energy storage applications.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-003', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BMS', standardRequirement: 'IEC 61508 — BMS', requirementLevel: 'Mandatory', applicabilityCondition: 'Functional-safety evidence for safety-related BMS functions under ST technical evaluation.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-004', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'EMS', standardRequirement: 'IEC 61508 — EMS', requirementLevel: 'Mandatory', applicabilityCondition: 'Functional-safety evidence for safety-related EMS functions where applicable.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-005', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS PCS', standardRequirement: 'IEC 62477-1 — PCS', requirementLevel: 'Mandatory', applicabilityCondition: 'Power conversion system safety.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-006', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS PCS', standardRequirement: 'IEC 62116 — PCS', requirementLevel: 'Mandatory', applicabilityCondition: 'Grid-connected PCS anti-islanding; mark N/A for standalone off-grid BESS.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Grid-connected only', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-007', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'EMS', standardRequirement: 'IEC 61850 — EMS', requirementLevel: 'Mandatory', applicabilityCondition: 'ST component evaluation reference for EMS; applicability depends on the supplied EMS/utility interface scope.', evidenceType: 'Functional Test', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-008', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'UN 38.3', requirementLevel: 'Mandatory', applicabilityCondition: 'Required before transport; verify test summary/report matches cell/module/pack transported configuration.', evidenceType: 'Test Summary', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-009', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'IEC 62933-5-1', requirementLevel: 'Mandatory', applicabilityCondition: 'System hazard identification, risk assessment and mitigation strategy.', evidenceType: 'Risk Assessment', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-010', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'IEC 62933-5-2', requirementLevel: 'Mandatory', applicabilityCondition: 'Electrical and electrochemical BESS system safety requirements.', evidenceType: 'Design Document', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-011', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'IEC 60529 / Outdoor IP54+', requirementLevel: 'Mandatory', applicabilityCondition: 'Outdoor BESS enclosures, cabinets and electrical equipment; minimum IP54 unless site requires higher.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-012', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'UL 9540A or authority-accepted fire test path', requirementLevel: 'Mandatory', applicabilityCondition: 'ST requires thermal-runaway protection design/testing to UL 9540A or relevant fire/safety authority requirements.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-013', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'MS IEC 62305', requirementLevel: 'Mandatory', applicabilityCondition: 'Lightning protection for applicable grid-connected and standalone BESS applications.', evidenceType: 'Design Document', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-014', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'Factory Acceptance Test (FAT)', requirementLevel: 'Mandatory', applicabilityCondition: 'FAT certificate listing tested components before installation.', evidenceType: 'FAT', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-015', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'Site Acceptance Test (SAT)', requirementLevel: 'Mandatory', applicabilityCondition: 'Post-installation electrical safety, protection, isolation and anti-islanding tests as applicable.', evidenceType: 'SAT', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-016', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'Form G — Supervision and Completion Certificate', requirementLevel: 'Mandatory', applicabilityCondition: 'Statutory Malaysian installation completion document signed by the competent person.', evidenceType: 'Statutory Form', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-017', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'Form H — Test Certificate', requirementLevel: 'Mandatory', applicabilityCondition: 'Statutory Malaysian installation test document signed by the competent person.', evidenceType: 'Statutory Form', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-018', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'IEC 62620', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Industrial secondary-cell/battery performance and endurance evaluation; does not replace IEC 62619/63056.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-019', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'UL 1973', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Stationary battery safety listing for lender/insurer or international project requirements.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.ul.com/services/energy-storage-system-testing-and-certification', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-020', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'UL 9540', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Complete ESS system certification; not interchangeable with UL 9540A.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.ul.com/services/energy-storage-system-testing-and-certification', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-021', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'NFPA 855', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Stationary ESS installation/fire protection where required by insurer, lender or contract.', evidenceType: 'Design Document', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.nfpa.org/codes-and-standards/nfpa-855-standard-development/855', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-022', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'NFPA 70', requirementLevel: 'Optional', applicabilityCondition: 'Reference for working clearances only where local authority requirements are absent, as stated by ST.', evidenceType: 'Design Document', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.st.gov.my/resources/guidelines-battery-energy-storage-system-bess-safety', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-023', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'IEC 62933-2-1', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'BESS performance parameters and test methods.', evidenceType: 'Test Report', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-024', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'IEC 62933-3 series', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Utility-scale planning, installation and system integration.', evidenceType: 'Design Document', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-025', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'BESS System', standardRequirement: 'IEC 62933-4 series', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Environmental and lifecycle considerations.', evidenceType: 'Design Document', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://webstore.iec.ch/', projectApplicability: 'Utility', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-026', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'ISO 9001', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Manufacturer quality management system; verify scope and production site.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.iso.org/iso-9001-quality-management.html', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-027', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'ISO 14001', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Manufacturer environmental management system.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.iso.org/iso-14001-environmental-management.html', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-028', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'ISO 45001', requirementLevel: 'International Finance Preferred', applicabilityCondition: 'Manufacturer occupational health and safety management system.', evidenceType: 'Certificate', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.iso.org/iso-45001-occupational-health-and-safety.html', projectApplicability: 'All', remarks: '', autoCompletenessCheck: 'Action Required' },
            { matrixRecordId: 'BESS-029', matrixSheet: 'Battery_BESS_Cert', matrixType: 'Battery / BESS', productCategory: 'Battery Pack / System', standardRequirement: 'SIRIM Battery / BESS Certification', requirementLevel: 'Utility Preferred', applicabilityCondition: 'Project/tender-specific unless an official mandatory order covers the exact product and scope.', evidenceType: 'Product Listing / License', isApplicable: 'Conditional', evidenceAvailable: 'No', expiryStatus: 'No Expiry', modelCoverageStatus: 'Not Checked', factoryCoverageStatus: 'Not Checked', bomCoverageStatus: 'Not Checked', verificationStatus: 'Not Reviewed', officialRegistryVerified: 'Not Checked', officialRegistrySourceUrl: 'https://www.sirim-qas.com.my/', projectApplicability: 'Project-specific', remarks: '', autoCompletenessCheck: 'Action Required' }
        ];
        const CERTIFICATION_CLASS_PROFILES_DEFAULTS = [
            { id: 'class_a1', label: 'Class A1 Grid-Tied PV Only', title: 'Grid-Tied PV Only / Solar ATAP', examples: ['Solar ATAP', 'Factory Rooftop', 'Warehouse Rooftop', 'Commercial Building'], gridConnected: true, bessRequired: false, gridFormingRequired: false, gensetIntegrationRequired: false, utilityComplianceRequired: false, productGroups: ['PV Module', 'Inverter / PCS', 'BOS'], mandatoryStandards: ['IEC 61215', 'IEC 61730', 'IEC 62109', 'IEC 61727', 'IEC 62116', 'IEC 61683'], preferredStandards: ['IEC 61000 EMC', 'IEC 61853', 'IEC 62941', 'SIRIM Product Certification'], internationalFinanceStandards: [], optionalStandards: ['IEC 61701', 'IEC 62716'], notApplicableStandards: [] },
            { id: 'class_a2', label: 'Class A2 Grid-Tied PV + Battery', title: 'Grid-Tied PV + Battery', examples: ['Solar ATAP + Battery', 'Factory Rooftop ESS', 'Warehouse Rooftop ESS'], gridConnected: true, bessRequired: true, gridFormingRequired: false, gensetIntegrationRequired: false, utilityComplianceRequired: false, productGroups: ['PV Module', 'Inverter / PCS', 'Battery', 'ESS', 'BOS'], mandatoryStandards: ['IEC 61215', 'IEC 61730', 'IEC 62109', 'IEC 61727', 'IEC 62116', 'IEC 61683', 'IEC 62619', 'IEC 63056', 'IEC 62477-1', 'UN 38.3', 'Project Scope Documentation'], preferredStandards: ['IEC 61508', 'IEC 61850', 'IEC 61000', 'IEC 61853', 'IEC TS 62804', 'IEC 62933'], internationalFinanceStandards: ['UL 9540', 'UL 9540A', 'UL 1973', 'NFPA 855'], optionalStandards: ['ISO 9001', 'ISO 14001', 'Regional Standards'], notApplicableStandards: [] },
            { id: 'class_b', label: 'Class B Hybrid PV+BESS', title: 'Hybrid PV + BESS', examples: ['Factory Peak Shaving', 'Data Center', 'Mall', 'Hospital'], gridConnected: true, bessRequired: true, gridFormingRequired: false, gensetIntegrationRequired: false, utilityComplianceRequired: true, productGroups: ['PV Module', 'Inverter / PCS', 'Battery', 'ESS', 'BOS'], mandatoryStandards: ['IEC 61215', 'IEC 61730', 'IEC 62109', 'IEC 61727', 'IEC 62116', 'IEC 61683', 'IEC 62477-1', 'IEC 62619', 'IEC 63056', 'UN 38.3', 'IEC 62933-5-1', 'IEC 62933-5-2'], preferredStandards: ['IEC 61508', 'IEC 61850', 'IEC 60529'], internationalFinanceStandards: ['UL 9540A', 'UL 9540', 'UL 1973', 'NFPA 855'], optionalStandards: ['ISO 9001', 'ISO 14001'], notApplicableStandards: [] },
            { id: 'class_c', label: 'Class C Off-Grid', title: 'Off-Grid PV + BESS + Genset', examples: ['Palm Oil Estate', 'Remote Plantation', 'Mining', 'Telecom Tower', 'Island Resort'], gridConnected: false, bessRequired: true, gridFormingRequired: true, gensetIntegrationRequired: true, utilityComplianceRequired: false, productGroups: ['PV Module', 'Inverter / PCS', 'Battery', 'ESS', 'BOS'], mandatoryStandards: ['IEC 61215', 'IEC 61730', 'IEC 62109', 'IEC 62477-1', 'IEC 62619', 'IEC 63056', 'UN 38.3', 'IEC 62933-5-1', 'IEC 62933-5-2', 'FAT', 'SAT'], preferredStandards: ['IEC 61508', 'UL 9540A'], notApplicableStandards: ['IEC 61727', 'IEC 62116', 'IEEE 1547'] },
            { id: 'class_d', label: 'Class D Utility Solar', title: 'Utility Scale Solar', examples: ['LSS', 'CGPP', 'Solar Farm'], gridConnected: true, bessRequired: false, gridFormingRequired: false, gensetIntegrationRequired: false, utilityComplianceRequired: true, productGroups: ['PV Module', 'Inverter / PCS', 'BOS'], mandatoryStandards: ['IEC 61215', 'IEC 61730', 'IEC 62109', 'IEC 61727', 'IEC 62116', 'IEC 61683'], preferredStandards: ['IEC 61000 EMC', 'IEC 61853', 'IEC 62941', 'IEEE 1547'], notApplicableStandards: [] },
            { id: 'class_e', label: 'Class E Utility BESS', title: 'Utility Scale BESS', examples: ['MyBEST', 'TNB BESS', 'Sabah BESS'], gridConnected: true, bessRequired: true, gridFormingRequired: true, gensetIntegrationRequired: false, utilityComplianceRequired: true, productGroups: ['Inverter / PCS', 'Battery', 'ESS', 'BOS'], mandatoryStandards: ['IEC 62619', 'IEC 63056', 'IEC 61508', 'IEC 61850', 'IEC 62477-1', 'IEC 62116', 'IEC 62933-5-1', 'IEC 62933-5-2', 'UN 38.3', 'FAT', 'SAT'], preferredStandards: ['UL 9540A', 'UL 9540', 'UL 1973', 'NFPA 855', 'IEC 60529'], notApplicableStandards: [] }
        ];
        ...
        function productMasterActionsHtml(product) {
            ...
            return `<div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                <button onclick="openProductCertificationMatrix('${id}')" class="text-amber-600 hover:bg-amber-50 p-1 rounded" title="Certification Matrix">✓</button>
        ...
        function certificationMatrixRecordKey(record = {}) {
            return String(record.matrixRecordId || record.recordId || record.id || record.code || record.standardCode || '').trim();
        }
        function normalizeCertificationStandard(record = {}) {
            const matrixRecordId = certificationMatrixRecordKey(record);
            const standardRequirement = String(record.standardRequirement || record.standard || record.standardCode || record.code || record.name || '').trim();
            if (!matrixRecordId && !standardRequirement) return null;
            const requirementLevel = CERTIFICATION_LEVELS.includes(record.requirementLevel) ? record.requirementLevel : (CERTIFICATION_LEVELS.includes(record.level) ? record.level : 'Optional');
            const evidenceType = String(record.evidenceType || record.documentType || 'Certificate / Report').trim();
            return {
                matrixRecordId: matrixRecordId || standardRequirement,
                matrixSheet: String(record.matrixSheet || '').trim(),
                matrixType: String(record.matrixType || record.productType || '').trim(),
                productCategory: String(record.productCategory || record.productType || '').trim(),
                standardRequirement,
                requirementLevel,
                applicabilityCondition: String(record.applicabilityCondition || '').trim(),
                evidenceType,
                isApplicable: String(record.isApplicable || 'Conditional').trim(),
                evidenceAvailable: String(record.evidenceAvailable || 'No').trim(),
                expiryStatus: String(record.expiryStatus || '').trim(),
                modelCoverageStatus: String(record.modelCoverageStatus || '').trim(),
                factoryCoverageStatus: String(record.factoryCoverageStatus || '').trim(),
                bomCoverageStatus: String(record.bomCoverageStatus || '').trim(),
                verificationStatus: String(record.verificationStatus || 'Not Reviewed').trim(),
                officialRegistryVerified: String(record.officialRegistryVerified || '').trim(),
                officialRegistrySourceUrl: String(record.officialRegistrySourceUrl || record.officialUrl || '').trim(),
                projectApplicability: String(record.projectApplicability || '').trim(),
                remarks: String(record.remarks || record.remark || '').trim(),
                autoCompletenessCheck: String(record.autoCompletenessCheck || '').trim(),
                code: standardRequirement,
                name: standardRequirement,
                level: requirementLevel,
                productType: String(record.matrixType || record.productType || '').trim(),
                documentType: evidenceType,
                requiredEvidence: String(record.requiredEvidence || record.evidenceType || '').trim()
            };
        }
        function normalizeCertificationStandardsCatalog(records = []) {
            const map = new Map();
            [...CERTIFICATION_STANDARDS_CATALOG_DEFAULTS, ...(Array.isArray(records) ? records : [])].forEach(record => {
                const normalized = normalizeCertificationStandard(record);
                if (normalized) map.set(normalized.matrixRecordId, normalized);
            });
        ...
        function normalizeCertificationClassProfile(profile = {}) {
            ...
            return {
                ...
                title: String(profile.title || profile.label || id).trim(),
                examples: Array.isArray(profile.examples) ? profile.examples.map(v => String(v || '').trim()).filter(Boolean) : [],
                gridConnected: !!profile.gridConnected,
                bessRequired: !!profile.bessRequired,
                gridFormingRequired: !!profile.gridFormingRequired,
                gensetIntegrationRequired: !!profile.gensetIntegrationRequired,
                utilityComplianceRequired: !!profile.utilityComplianceRequired,
                productGroups: Array.isArray(profile.productGroups) ? profile.productGroups.map(v => String(v || '').trim()).filter(Boolean) : [],
                mandatoryStandards: Array.isArray(profile.mandatoryStandards) ? profile.mandatoryStandards.map(v => String(v || '').trim()).filter(Boolean) : [],
                preferredStandards: Array.isArray(profile.preferredStandards) ? profile.preferredStandards.map(v => String(v || '').trim()).filter(Boolean) : [],
                internationalFinanceStandards: Array.isArray(profile.internationalFinanceStandards) ? profile.internationalFinanceStandards.map(v => String(v || '').trim()).filter(Boolean) : [],
                optionalStandards: Array.isArray(profile.optionalStandards) ? profile.optionalStandards.map(v => String(v || '').trim()).filter(Boolean) : [],
                notApplicableStandards: Array.isArray(profile.notApplicableStandards) ? profile.notApplicableStandards.map(v => String(v || '').trim()).filter(Boolean) : [],
                levelOverrides: profile.levelOverrides && typeof profile.levelOverrides === 'object' ? { ...profile.levelOverrides } : {}
            };
        }
        function normalizeCertificationClassProfiles(records = []) {
            const map = new Map();
            [...CERTIFICATION_CLASS_PROFILES_DEFAULTS, ...(Array.isArray(records) ? records : [])].forEach(record => {
                const normalized = normalizeCertificationClassProfile(record);
                if (normalized) map.set(normalized.id, normalized);
            });
            return Array.from(map.values());
        }
        function normalizeProductCertificationEvidence(record = {}) {
            const productId = String(record.productId || record.productID || record.sku || '').trim();
            const matrixRecordId = certificationMatrixRecordKey(record);
            const lookupKey = String(matrixRecordId || record.standardRequirement || record.standardCode || record.code || '').trim();
            const standard = (Array.isArray(certificationStandardsCatalog) ? certificationStandardsCatalog : [])
                .find(item => item?.matrixRecordId === lookupKey || item?.standardRequirement === lookupKey || item?.code === lookupKey);
            const standardRequirement = String(record.standardRequirement || standard?.standardRequirement || record.standardCode || record.code || '').trim();
            const id = String(record.id || [productId, matrixRecordId || standardRequirement].filter(Boolean).join('_')).trim();
            if (!productId && !matrixRecordId && !standardRequirement) return null;
            const verificationStatus = String(record.verificationStatus || record.status || record.evidenceStatus || 'Not Reviewed').trim();
            return {
                id,
                productId,
                matrixRecordId: matrixRecordId || standard?.matrixRecordId || standardRequirement,
                standardRequirement,
                requirementLevel: String(record.requirementLevel || standard?.requirementLevel || '').trim(),
                evidenceType: String(record.evidenceType || record.documentType || standard?.evidenceType || '').trim(),
                isApplicable: String(record.isApplicable || standard?.isApplicable || 'Conditional').trim(),
                evidenceAvailable: String(record.evidenceAvailable || '').trim(),
                certificateNo: String(record.certificateNo || '').trim(),
                reportNo: String(record.reportNo || record.reportNumber || '').trim(),
                certificationListingBody: String(record.certificationListingBody || record.certificationBody || '').trim(),
                testingLaboratory: String(record.testingLaboratory || record.testLab || record.laboratory || '').trim(),
                laboratoryAccreditationScope: String(record.laboratoryAccreditationScope || '').trim(),
                standardEditionVersion: String(record.standardEditionVersion || '').trim(),
                issueDate: String(record.issueDate || '').trim(),
                expiryDate: String(record.expiryDate || record.expiry || '').trim(),
                expiryStatus: String(record.expiryStatus || '').trim(),
                modelCoverageStatus: String(record.modelCoverageStatus || '').trim(),
                modelCoverageDetails: String(record.modelCoverageDetails || record.applicableModels || '').trim(),
                factoryCoverageStatus: String(record.factoryCoverageStatus || '').trim(),
                factoryProductionSite: String(record.factoryProductionSite || record.factoryScope || '').trim(),
                bomCoverageStatus: String(record.bomCoverageStatus || '').trim(),
                bomRevisionCriticalComponents: String(record.bomRevisionCriticalComponents || '').trim(),
                reportScopeKeyResult: String(record.reportScopeKeyResult || '').trim(),
                verificationStatus,
                verifiedBy: String(record.verifiedBy || '').trim(),
                lastValidationDate: String(record.lastValidationDate || '').trim(),
                nextReviewDate: String(record.nextReviewDate || '').trim(),
                officialRegistryVerified: String(record.officialRegistryVerified || '').trim(),
                officialRegistrySourceUrl: String(record.officialRegistrySourceUrl || standard?.officialRegistrySourceUrl || '').trim(),
                documentLink: String(record.documentLink || record.evidenceLink || record.link || record.url || '').trim(),
                projectApplicability: String(record.projectApplicability || standard?.projectApplicability || '').trim(),
                linkedProjectId: String(record.linkedProjectId || '').trim(),
                deviationNcr: String(record.deviationNcr || '').trim(),
                remarks: String(record.remarks || record.remark || '').trim(),
                status: verificationStatus === 'Verified' ? 'Ready' : (verificationStatus === 'Expired' ? 'Expired' : (verificationStatus === 'Conditional Acceptance' ? 'Conditional' : 'Needs Review')),
                ts: record.ts || Date.now()
            };
        }
        function normalizeProductCertificationEvidenceList(records = []) {
            return (Array.isArray(records) ? records : []).map(normalizeProductCertificationEvidence).filter(Boolean);
        }
        function ensureCertificationV4Data() {
            certificationStandardsCatalog = normalizeCertificationStandardsCatalog(certificationStandardsCatalog);
            certificationClassProfiles = normalizeCertificationClassProfiles(certificationClassProfiles);
            productCertificationEvidence = normalizeProductCertificationEvidenceList(productCertificationEvidence);
        }
        function getCertificationClassProfile(classId = '') {
            ensureCertificationV4Data();
            const id = String(classId || getSelectedCertificationClassId()).trim();
            return certificationClassProfiles.find(profile => profile.id === id) || certificationClassProfiles[0] || normalizeCertificationClassProfile(CERTIFICATION_CLASS_PROFILES_DEFAULTS[0]);
        }
        function getCertificationStandard(code = '') {
            ensureCertificationV4Data();
            const q = String(code || '').trim();
            return certificationStandardsCatalog.find(standard => standard.matrixRecordId === q || standard.standardRequirement === q || standard.code === q) || null;
        }
        function isCertificationStandardApplicableToClass(standard, profile) {
            const code = String(standard?.standardRequirement || standard?.code || standard || '').trim();
            const project = String(standard?.projectApplicability || '').toLowerCase();
            const condition = String(standard?.applicabilityCondition || '').toLowerCase();
            if (!code || !profile) return true;
            if ((profile.notApplicableStandards || []).includes(code)) return false;
            if (!profile.gridConnected && (project.includes('grid-connected') || condition.includes('grid-connected') || condition.includes('anti-islanding'))) return false;
            if (profile.gridConnected && project.includes('off-grid only')) return false;
            return true;
        }
        function certificationMatrixTypesForClass(profile = {}) {
            const groups = new Set(profile.productGroups || []);
            const out = [];
            if (groups.has('PV Module')) out.push('PV Module');
            if (groups.has('Inverter / PCS')) out.push('Inverter / PCS');
            if (groups.has('Battery') || groups.has('ESS')) out.push('Battery / BESS');
            return out;
        }
        function getCertificationChecklistForClass(classId) {
            const profile = getCertificationClassProfile(classId);
            const matrixTypes = certificationMatrixTypesForClass(profile);
            return certificationStandardsCatalog.filter(standard => matrixTypes.includes(standard.matrixType)).map(standard => {
                const notApplicable = !isCertificationStandardApplicableToClass(standard, profile);
                const preferred = ['Utility Preferred', 'International Finance Preferred', 'Optional'].includes(standard.requirementLevel);
                const mandatory = standard.requirementLevel === 'Mandatory';
                return {
                    ...standard,
                    level: notApplicable ? 'N/A' : (mandatory ? 'Mandatory' : (preferred ? standard.level : standard.level)),
                    status: notApplicable ? 'N/A' : 'Needs Review',
                    notApplicable,
                    reason: notApplicable ? `${profile.label}: ${standard.projectApplicability || 'condition'} not applicable` : ''
                };
            });
        }
        function getProductCertificationEvidence(productId) {
            ...
            if (!id) return [];
            ensureCertificationV4Data();
            return productCertificationEvidence.filter(record => record.productId === id);
        }
        function certificationMatrixTypesForProduct(product = {}) {
            const group = getProductTypeGroup(product).id;
            if (group === 'pv') return ['PV Module'];
            if (group === 'inverter') return ['Inverter / PCS'];
            if (group === 'battery') return ['Battery / BESS'];
            if (group === 'ess') return ['Inverter / PCS', 'Battery / BESS'];
            return [];
        }
        function getCertificationMatrixRowsForProduct(product = {}) {
            ensureCertificationV4Data();
            const types = certificationMatrixTypesForProduct(product);
            return certificationStandardsCatalog.filter(record => types.includes(record.matrixType));
        }
        function renderProductCertificationMatrixSummary(productOrId = '') {
            const product = typeof productOrId === 'string'
                ? products.find(p => String(p.id || '') === String(productOrId || ''))
                : productOrId;
            const box = document.getElementById('m-cert-matrix-summary');
            if (!box || !product) return;
            const rows = getCertificationMatrixRowsForProduct(product);
            const evidence = getProductCertificationEvidence(product.id);
            const selected = evidence.filter(item => rows.some(row => row.matrixRecordId === item.matrixRecordId));
            const verified = selected.filter(item => item.verificationStatus === 'Verified' || item.status === 'Ready').length;
            const mandatory = rows.filter(row => row.requirementLevel === 'Mandatory').length;
            box.innerHTML = `<div class="flex flex-wrap items-center gap-2">
                ${productMasterStatusPill(`${selected.length} selected`, selected.length ? 'purple' : 'slate')}
                ${productMasterStatusPill(`${verified} verified`, verified ? 'green' : 'slate')}
                ${productMasterStatusPill(`${mandatory} mandatory`, 'amber')}
                <button type="button" onclick="openProductCertificationMatrix('${htmlSafe(product.id || '')}')" class="ml-auto text-xs font-black text-purple-700 hover:underline">Edit matrix evidence</button>
            </div>`;
        }
        function updateCertificationMatrixFilters(rows = []) {
            const configs = [
                ['cert-matrix-level-filter', 'requirementLevel', 'All Levels'],
                ['cert-matrix-project-filter', 'projectApplicability', 'All Project Applicability'],
                ['cert-matrix-evidence-filter', 'evidenceType', 'All Evidence Types']
            ];
            configs.forEach(([id, key, label]) => {
                ...
                el.innerHTML = `<option value="">${label}</option>` + values.map(value => `<option value="${htmlSafe(value)}">${htmlSafe(value)}</option>`).join('');
                if (keep && values.includes(keep)) el.value = keep;
            });
        }
        function certificationMatrixEvidenceInput(row, evidence, key, label, type = 'text') {
            const rid = safeDomId(row.matrixRecordId);
            const value = evidence?.[key] || '';
            return `<label class="block">
                <span class="block text-[9px] font-black text-slate-400 uppercase mb-1">${htmlSafe(label)}</span>
                <input id="cert-matrix-${rid}-${key}" type="${type}" value="${htmlSafe(value)}" class="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs outline-none focus:border-purple-500 bg-white">
            </label>`;
        }
        function renderProductCertificationMatrix() {
            const productId = String(window.__certMatrixProductId || window.editId || '').trim();
            const product = products.find(p => String(p.id || '') === productId) || (window.editId ? products.find(p => p.id === window.editId) : null) || {
                id: productId,
                category: document.getElementById('m-category')?.value || '',
                name: document.getElementById('m-name')?.value || ''
            };
            const list = document.getElementById('cert-matrix-list');
            if (!list) return;
            const allRows = getCertificationMatrixRowsForProduct(product);
            updateCertificationMatrixFilters(allRows);
            const q = String(document.getElementById('cert-matrix-search')?.value || '').trim().toLowerCase();
            const level = String(document.getElementById('cert-matrix-level-filter')?.value || '').trim();
            const project = String(document.getElementById('cert-matrix-project-filter')?.value || '').trim();
            const evidenceType = String(document.getElementById('cert-matrix-evidence-filter')?.value || '').trim();
            const evidence = new Map(getProductCertificationEvidence(product.id).map(item => [item.matrixRecordId, item]));
            const rows = allRows.filter(row => {
                const text = `${row.matrixRecordId} ${row.productCategory} ${row.standardRequirement} ${row.requirementLevel} ${row.applicabilityCondition} ${row.evidenceType} ${row.projectApplicability}`.toLowerCase();
                if (q && !text.includes(q)) return false;
                if (level && row.requirementLevel !== level) return false;
                if (project && row.projectApplicability !== project) return false;
                if (evidenceType && row.evidenceType !== evidenceType) return false;
                return true;
            });
            const summary = document.getElementById('cert-matrix-summary');
            const selectedCount = [...evidence.keys()].filter(id => allRows.some(row => row.matrixRecordId === id)).length;
            if (summary) summary.textContent = `${selectedCount} selected / ${rows.length} visible`;
            list.innerHTML = rows.length ? rows.map(row => {
                const ev = evidence.get(row.matrixRecordId);
                const rid = safeDomId(row.matrixRecordId);
                const checked = !!ev;
                return `<div class="rounded-2xl border ${checked ? 'border-purple-200 bg-purple-50/30' : 'border-slate-200 bg-white'} p-4">
                    <div class="flex flex-col lg:flex-row lg:items-start gap-3">
                        <label class="flex items-start gap-3 min-w-0 flex-1">
                            <input id="cert-matrix-${rid}-checked" data-matrix-record-id="${htmlSafe(row.matrixRecordId)}" type="checkbox" class="cert-matrix-row-check mt-1 h-4 w-4 accent-purple-700" ${checked ? 'checked' : ''}>
                            <span class="min-w-0">
                                <span class="block text-xs font-black text-slate-800">${htmlSafe(row.matrixRecordId)} · ${htmlSafe(row.standardRequirement)}</span>
                                <span class="block text-[10px] text-slate-500 mt-1">${htmlSafe(row.productCategory)} · ${htmlSafe(row.evidenceType)} · ${htmlSafe(row.projectApplicability || 'Project-specific')}</span>
                                <span class="block text-[10px] text-slate-400 mt-1">${htmlSafe(row.applicabilityCondition || '-')}</span>
                            </span>
                        </label>
                        <div class="flex flex-wrap gap-1.5">
                            ${productMasterStatusPill(row.requirementLevel, certificationLevelTone(row.requirementLevel))}
                            ${productMasterStatusPill(ev?.verificationStatus || row.verificationStatus || 'Not Reviewed', ev?.verificationStatus === 'Verified' ? 'green' : 'slate')}
                        </div>
                    </div>
                    <details class="cert-matrix-evidence-fields mt-3 rounded-xl border border-slate-200 bg-white/80">
                        <summary class="cursor-pointer select-none px-3 py-2 text-[11px] font-black text-purple-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                            <span>Evidence fields</span>
                            <span class="text-[10px] font-bold text-slate-400">Applicable / Evidence / Verification / Certificate No. / Report No. / Expiry Date / Body / Testing Lab / Version / Registry URL / Document Link / Remarks</span>
                        </summary>
                        <div class="grid grid-cols-1 md:grid-cols-4 xl:grid-cols-6 gap-2 p-3 border-t border-slate-100">
                            <label class="block">
                                <span class="block text-[9px] font-black text-slate-400 uppercase mb-1">Applicable</span>
                                <select id="cert-matrix-${rid}-isApplicable" class="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white">
                                    ${['Conditional', 'Yes', 'No', 'Not Applicable'].map(value => `<option value="${value}" ${(ev?.isApplicable || row.isApplicable) === value ? 'selected' : ''}>${value}</option>`).join('')}
                                </select>
                            </label>
                            <label class="block">
                                <span class="block text-[9px] font-black text-slate-400 uppercase mb-1">Evidence</span>
                                <select id="cert-matrix-${rid}-evidenceAvailable" class="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white">
                                    ${['No', 'Yes', 'Partial'].map(value => `<option value="${value}" ${(ev?.evidenceAvailable || row.evidenceAvailable) === value ? 'selected' : ''}>${value}</option>`).join('')}
                                </select>
                            </label>
                            <label class="block">
                                <span class="block text-[9px] font-black text-slate-400 uppercase mb-1">Verification</span>
                                <select id="cert-matrix-${rid}-verificationStatus" class="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white">
                                    ${['Not Reviewed', 'Pending Evidence', 'Under Review', 'Verified', 'Conditional Acceptance', 'Rejected', 'Expired'].map(value => `<option value="${value}" ${(ev?.verificationStatus || row.verificationStatus) === value ? 'selected' : ''}>${value}</option>`).join('')}
                                </select>
                            </label>
                            ${certificationMatrixEvidenceInput(row, ev, 'certificateNo', 'Certificate No.')}
                            ${certificationMatrixEvidenceInput(row, ev, 'reportNo', 'Report No.')}
                            ${certificationMatrixEvidenceInput(row, ev, 'expiryDate', 'Expiry Date', 'date')}
                            ${certificationMatrixEvidenceInput(row, ev, 'certificationListingBody', 'Body')}
                            ${certificationMatrixEvidenceInput(row, ev, 'testingLaboratory', 'Testing Lab')}
                            ${certificationMatrixEvidenceInput(row, ev, 'standardEditionVersion', 'Version')}
                            ${certificationMatrixEvidenceInput(row, ev, 'officialRegistrySourceUrl', 'Registry URL')}
                            ${certificationMatrixEvidenceInput(row, ev, 'documentLink', 'Document Link')}
                            ${certificationMatrixEvidenceInput(row, ev, 'remarks', 'Remarks')}
                        </div>
                    </details>
                </div>`;
            }).join('') : '<div class="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-400">No certification matrix rows match this product or filter.</div>';
        }
        window.renderProductCertificationMatrix = renderProductCertificationMatrix;
        window.openProductCertificationMatrix = (productId = '') => {
            const id = String(productId || window.editId || '').trim();
            if (!id && !document.getElementById('m-category')?.value) return alert('Please select a product category before opening the certification matrix.');
            window.__certMatrixProductId = id;
            const product = products.find(p => String(p.id || '') === id) || { id, name: document.getElementById('m-name')?.value || 'New Product', category: document.getElementById('m-category')?.value || '' };
            const title = document.getElementById('cert-matrix-title');
            const subtitle = document.getElementById('cert-matrix-subtitle');
            if (title) title.textContent = `${product.id || 'New Product'} · ${productListDisplayText(product.name || '')}`;
            if (subtitle) subtitle.textContent = `${productListDisplayText(product.category || '')} certification evidence matrix`;
            document.getElementById('certification-matrix-modal')?.classList.remove('hidden');
            renderProductCertificationMatrix();
        };
        window.closeProductCertificationMatrix = () => {
            document.getElementById('certification-matrix-modal')?.classList.add('hidden');
        };
        window.saveProductCertificationMatrix = () => {
            const productId = String(window.__certMatrixProductId || window.editId || '').trim();
            if (!productId) return alert('Please save the product record before saving certification evidence.');
            ensureCertificationV4Data();
            const rows = getCertificationMatrixRowsForProduct(products.find(p => String(p.id || '') === productId) || { id: productId, category: document.getElementById('m-category')?.value || '' });
            const rowIds = new Set(rows.map(row => row.matrixRecordId));
            const previousEvidenceIds = productCertificationEvidence
                .filter(item => item.productId === productId && rowIds.has(item.matrixRecordId))
                .map(item => item.id);
            productCertificationEvidence = productCertificationEvidence.filter(item => item.productId !== productId || !rowIds.has(item.matrixRecordId));
            Array.from(document.querySelectorAll('.cert-matrix-row-check')).forEach(check => {
                if (!check.checked) return;
                const matrixRecordId = String(check.dataset.matrixRecordId || '').trim();
                const row = rows.find(item => item.matrixRecordId === matrixRecordId);
                if (!row) return;
                const rid = safeDomId(matrixRecordId);
                const read = key => document.getElementById(`cert-matrix-${rid}-${key}`)?.value || '';
                productCertificationEvidence.push(normalizeProductCertificationEvidence({
                    id: `${productId}_${matrixRecordId}`,
                    productId,
                    matrixRecordId,
                    standardRequirement: row.standardRequirement,
                    requirementLevel: row.requirementLevel,
                    evidenceType: row.evidenceType,
                    isApplicable: read('isApplicable') || row.isApplicable,
                    evidenceAvailable: read('evidenceAvailable') || row.evidenceAvailable,
                    certificateNo: read('certificateNo'),
                    reportNo: read('reportNo'),
                    certificationListingBody: read('certificationListingBody'),
                    testingLaboratory: read('testingLaboratory'),
                    standardEditionVersion: read('standardEditionVersion'),
                    expiryDate: read('expiryDate'),
                    verificationStatus: read('verificationStatus') || 'Not Reviewed',
                    officialRegistrySourceUrl: read('officialRegistrySourceUrl') || row.officialRegistrySourceUrl,
                    documentLink: read('documentLink'),
                    projectApplicability: row.projectApplicability,
                    remarks: read('remarks')
                }));
            });
            saveToLocal();
            const currentEvidence = getProductCertificationEvidence(productId);
            const currentIds = new Set(currentEvidence.map(record => record.id));
            previousEvidenceIds.filter(id => !currentIds.has(id)).forEach(id => deleteEntityFromD1('certification_evidence', id));
            currentEvidence.forEach(record => {
                persistEntityToD1('certification_evidence', record.id, record);
            });
            renderProductCertificationMatrixSummary(productId);
            try { renderDb(); } catch (e) {}
            try { renderCertificationClassWorkspace(); } catch (e) {}
            window.closeProductCertificationMatrix();
        };
        function certificationTextIncludes(product, code) {
            const q = String(code || '').toLowerCase();
            if (!q) return false;
            const req = getProductCertificationRequirements(product);
            const md = getProductMasterData(product);
            const attached = [...productMasterAttachedCertFiles(product), ...productMasterAttachedSpecFiles(product)];
            const haystack = [
                ...(req.standards || []),
                ...(req.countries || []),
                md.certificateLink,
        ...
        function standardMatchesProductType(standard, product) {
            const category = normalizeProductCategory(product?.category || '');
            const group = getProductTypeGroup(product).id;
            const type = String(standard?.productType || '').toLowerCase();
            if (!type) return true;
            if (group === 'pv') return type.includes('pv');
            if (group === 'inverter') return type.includes('inverter') || type.includes('pcs');
            if (group === 'battery') return type.includes('battery') || type.includes('bess');
            if (group === 'ess') return type.includes('battery') || type.includes('bess') || type.includes('pcs') || type.includes('ems');
            if (category === 'Accessory') return type.includes('bos') || type.includes('project') || type.includes('site');
            return true;
        }
        function getProductCertificationV4Status(product, classId = '') {
            const profile = getCertificationClassProfile(classId);
            const checklist = getCertificationChecklistForClass(profile.id)
                .filter(item => item.level === 'Mandatory')
                .filter(item => standardMatchesProductType(item, product));
            if (!checklist.length) return { status: 'N/A', tone: 'slate', missing: [], label: `${profile.label}: no mandatory product evidence`, checklist: [] };
            const evidence = getProductCertificationEvidence(product?.id || '');
            const missing = [];
            let conditional = false;
            let expired = false;
            checklist.forEach(item => {
                if (!isCertificationStandardApplicableToClass(item, profile)) return;
                const match = evidence.find(record => record.matrixRecordId === item.matrixRecordId);
                if (match) {
                    if (match.status === 'Expired' || match.verificationStatus === 'Expired') expired = true;
                    if (match.status === 'Conditional' || match.status === 'Needs Review' || match.verificationStatus === 'Not Reviewed') conditional = true;
                    if (match.status === 'Gap' || match.evidenceAvailable === 'No') missing.push(item.standardRequirement);
                    return;
                }
                if (certificationTextIncludes(product, item.standardRequirement)) {
                    conditional = true;
                    return;
                }
                missing.push(item.standardRequirement);
            });
            if (expired) return { status: 'Expired', tone: 'red', missing, label: `${missing.length} missing / expired`, checklist };
            if (missing.length) return { status: 'Gap', tone: 'amber', missing, label: `${missing.length} missing`, checklist };
            if (conditional) return { status: 'Conditional', tone: 'amber', missing: [], label: 'Legacy evidence needs review', checklist };
            return { status: 'Ready', tone: 'green', missing: [], label: 'Mandatory evidence ready', checklist };
        }
        function getSelectedCertificationClassId() {
            try {
                const stored = localStorage.getItem('minova_certification_class_view_v1') || '';
                if (stored) return stored;
            } catch (e) {}
            return 'class_a1';
        }
        window.setCertificationClassSelection = (classId) => {
            const id = String(classId || 'class_a1').trim();
            try { localStorage.setItem('minova_certification_class_view_v1', id); } catch (e) {}
            renderCertificationClassWorkspace();
            try { renderDb(); } catch (e) {}
        };
        function certificationLevelTone(level, status = '') {
            if (status === 'Ready') return 'green';
            if (status === 'Gap' || status === 'Conditional' || status === 'Needs Review') return 'amber';
            if (status === 'Expired') return 'red';
            if (level === 'Mandatory') return 'purple';
            return 'slate';
        }
        function productMasterMissingEvidenceHtml(product) {
            const status = getProductCertificationV4Status(product, getSelectedCertificationClassId());
            const text = status.missing?.length ? status.missing.slice(0, 3).join(', ') : status.label;
            return `<span class="text-xs text-slate-600 max-w-[220px] block truncate" title="${htmlSafe(status.missing?.join(', ') || status.label)}">${htmlSafe(text || '-')}</span>`;
        }
        function productMasterClassFitHtml(product) {
            const profile = getCertificationClassProfile(getSelectedCertificationClassId());
            const groupLabel = getProductTypeGroup(product).label;
            const expected = (profile.productGroups || []).some(group => groupLabel.includes(group.split(' ')[0]) || group.includes(groupLabel) || (group === 'Inverter / PCS' && groupLabel === 'Inverter') || (group === 'ESS' && groupLabel.includes('ESS')));
            return productMasterStatusPill(expected ? profile.label.replace(/^Class\s+/i, '') : 'Not Primary', expected ? 'purple' : 'slate');
        }
        function productMasterCertificationV4StatusHtml(product) {
            const status = getProductCertificationV4Status(product, getSelectedCertificationClassId());
            return `${productMasterStatusPill(status.status, status.tone)}<div class="text-[10px] text-slate-400 mt-1">${htmlSafe(status.label)}</div>`;
        }
        function productMasterEvidenceCountHtml(product) {
            const count = getProductCertificationEvidence(product?.id || '').length;
            return `<button type="button" onclick="openProductCertificationMatrix('${htmlSafe(product?.id || '')}')" class="text-xs font-black text-purple-700 hover:underline">${count}</button><div class="text-[10px] text-slate-400">matrix rows</div>`;
        }
        ...
        function productMasterContext(product) {
            ...
            const certStatusV4 = getProductCertificationV4Status(product, getSelectedCertificationClassId());
            const certStatus = {
                status: certStatusV4.status,
                label: certStatusV4.label,
                gap: certStatusV4.missing?.length ? certStatusV4.missing.join(', ') : ''
            };
        ...
        const PRODUCT_MASTER_COLUMNS = {
            ...
            classFit: { key: 'classFit', label: 'Class Fit', render: p => productMasterClassFitHtml(p) },
            certificationV4Status: { key: 'certificationV4Status', label: 'Certification Status', render: p => productMasterCertificationV4StatusHtml(p) },
            missingEvidence: { key: 'missingEvidence', label: 'Missing Evidence', render: p => productMasterMissingEvidenceHtml(p) },
            evidenceCount: { key: 'evidenceCount', label: 'Evidence Count', align: 'text-center', render: p => productMasterEvidenceCountHtml(p) },
            ...
            externalCertificateLink: { key: 'externalCertificateLink', label: 'External Evidence Link', render: (p, ctx) => productMasterLinkHtml(ctx.masterData.certificateLink, 'Evidence') },
        ...
        function certificationClassConditionHtml(profile) {
            const rows = [
                ['Grid Connected', profile.gridConnected],
                ['BESS Required', profile.bessRequired],
                ['Grid Forming Required', profile.gridFormingRequired],
                ['Genset Integration Required', profile.gensetIntegrationRequired],
                ['Utility Compliance Required', profile.utilityComplianceRequired]
            ];
            return `<div>
                <div class="text-[10px] font-black text-purple-700 uppercase tracking-widest mb-1">${htmlSafe(profile.id.replace('_', ' ').toUpperCase())}</div>
                <h3 class="text-lg font-black text-slate-800">${htmlSafe(profile.title)}</h3>
                <p class="text-xs text-slate-400 mt-1">${htmlSafe((profile.examples || []).join(' / '))}</p>
                <div class="mt-4 space-y-2">
                    ${rows.map(([label, on]) => `<div class="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2">
                        <span class="text-xs font-bold text-slate-500">${htmlSafe(label)}</span>
                        ${productMasterStatusPill(on ? 'Yes' : 'No', on ? 'purple' : 'slate')}
                    </div>`).join('')}
                </div>
            </div>`;
        }
        function canEditCertificationStandards() {
            const auth = window.__minovaAuth || {};
            if (!auth.state?.user && window.location?.protocol === 'file:') return true;
            const role = String(auth.state?.permission?.role || auth.state?.user?.role || '').trim().toLowerCase();
            return role === 'admin' || role === 'operation_management';
        }
        function certificationClassStandardTokens(profile = {}) {
            return [
                ...(profile.mandatoryStandards || []),
                ...(profile.preferredStandards || []),
                ...(profile.internationalFinanceStandards || []),
                ...(profile.optionalStandards || [])
            ].filter(Boolean);
        }
        function certificationClassIncludesStandard(profile = {}, record = {}) {
            const id = String(record.matrixRecordId || '').trim();
            const standard = String(record.standardRequirement || '').trim().toLowerCase();
            return certificationClassStandardTokens(profile).some(token => {
                const q = String(token || '').trim().toLowerCase();
                return q && (q === id.toLowerCase() || q === standard || standard.includes(q) || q.includes(standard));
            });
        }
        function certificationClassLevelForStandard(profile = {}, record = {}) {
            const override = profile.levelOverrides?.[record.matrixRecordId];
            if (override) return override;
            const id = String(record.matrixRecordId || '').trim().toLowerCase();
            const standard = String(record.standardRequirement || '').trim().toLowerCase();
            const hasToken = list => (list || []).some(token => {
                const q = String(token || '').trim().toLowerCase();
                return q && (q === id || q === standard || standard.includes(q) || q.includes(standard));
            });
            if (hasToken(profile.mandatoryStandards)) return 'Mandatory';
            if (hasToken(profile.preferredStandards)) return 'Utility Preferred';
            if (hasToken(profile.internationalFinanceStandards)) return 'International Finance Preferred';
            if (hasToken(profile.optionalStandards)) return 'Optional';
            return record.requirementLevel || 'Optional';
        }
        function recordsForCertificationClass(profile = {}) {
            ensureCertificationV4Data();
            const rows = certificationStandardsCatalog.filter(record => certificationClassIncludesStandard(profile, record));
            const seen = new Set();
            return rows.map(record => ({ ...record, requirementLevel: certificationClassLevelForStandard(profile, record) }))
                .filter(record => {
                    const key = record.matrixRecordId || record.standardRequirement;
                    if (seen.has(key)) return false;
                    seen.add(key);
                    return true;
                });
        }
        function certificationStandardLevelClass(level = '') {
            const text = String(level || '').trim();
            if (text === 'Mandatory') return 'bg-red-50 text-red-700 border-red-100';
            if (text === 'Utility Preferred') return 'bg-amber-50 text-amber-700 border-amber-100';
            if (text === 'International Finance Preferred') return 'bg-purple-50 text-purple-700 border-purple-100';
            return 'bg-emerald-50 text-emerald-700 border-emerald-100';
        }
        const CERT_CLASS_LEVEL_FILTER_STORAGE_KEY = 'minova_cert_class_level_filters_v1';
        function getCertificationClassLevelFilters() {
            try {
                const parsed = JSON.parse(localStorage.getItem(CERT_CLASS_LEVEL_FILTER_STORAGE_KEY) || '[]');
                const valid = Array.isArray(parsed) ? parsed.filter(level => CERTIFICATION_LEVELS.includes(level)) : [];
                return valid.length ? valid : [...CERTIFICATION_LEVELS];
            } catch (e) {
                return [...CERTIFICATION_LEVELS];
            }
        }
        function setCertificationClassLevelFilters(levels = []) {
            const valid = Array.isArray(levels) ? levels.filter(level => CERTIFICATION_LEVELS.includes(level)) : [];
            localStorage.setItem(CERT_CLASS_LEVEL_FILTER_STORAGE_KEY, JSON.stringify(valid.length ? valid : CERTIFICATION_LEVELS));
        }
        window.toggleCertificationClassLevelFilter = (level = '') => {
            const value = String(level || '').trim();
            if (!CERTIFICATION_LEVELS.includes(value)) return;
            const selected = new Set(getCertificationClassLevelFilters());
            if (selected.has(value) && selected.size > 1) selected.delete(value);
            else selected.add(value);
            setCertificationClassLevelFilters([...selected]);
            renderCertificationClassWorkspace();
        };
        function renderCertificationClassLevelFilters() {
            const box = document.getElementById('cert-class-level-filters');
            if (!box) return;
            const selected = new Set(getCertificationClassLevelFilters());
            box.innerHTML = CERTIFICATION_LEVELS.map(level => {
                const active = selected.has(level);
                const tone = certificationStandardLevelClass(level);
                return `<button type="button" onclick="toggleCertificationClassLevelFilter('${htmlSafe(level)}')" class="inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-black ${active ? tone : 'bg-white text-slate-400 border-slate-200'}">
                    <span class="inline-flex h-3 w-3 items-center justify-center rounded-full border ${active ? 'border-current bg-white/70' : 'border-slate-200'}">${active ? '✓' : ''}</span>
                    ${htmlSafe(level)}
                </button>`;
            }).join('');
        }
        function certificationMatrixGroupLabel(type = '') {
            const value = String(type || '').trim();
            if (value === 'PV Module') return 'PV Module';
            if (value === 'Inverter / PCS') return 'Inverter / PCS';
            if (value === 'Battery / BESS') return 'Battery / BESS';
            return value || 'Other';
        }
        function certificationStandardRowHtml(record, canEdit = false) {
            const rid = htmlSafe(record.matrixRecordId || '');
            const registry = String(record.officialRegistrySourceUrl || '').trim();
            return `<tr class="border-b border-cyan-200/80 odd:bg-cyan-50 even:bg-white hover:bg-purple-50/40">
                <td class="py-3 px-3 font-mono text-xs text-slate-700">${rid || '-'}</td>
                <td class="py-3 px-3 text-xs text-slate-700">${htmlSafe(record.productCategory || record.matrixType || '-')}</td>
                <td class="py-3 px-3 text-xs font-bold text-slate-800 max-w-[260px] whitespace-normal">${htmlSafe(record.standardRequirement || '-')}</td>
                <td class="py-3 px-3"><span class="inline-flex rounded-lg border px-2 py-1 text-[10px] font-black ${certificationStandardLevelClass(record.requirementLevel)}">${htmlSafe(record.requirementLevel || 'Optional')}</span></td>
                <td class="py-3 px-3 text-xs text-slate-600 max-w-[340px] whitespace-normal">${htmlSafe(record.applicabilityCondition || '-')}</td>
                <td class="py-3 px-3 text-xs text-slate-700">${htmlSafe(record.evidenceType || '-')}</td>
                <td class="py-3 px-3 text-xs text-slate-700">${htmlSafe(record.projectApplicability || '-')}</td>
                <td class="py-3 px-3 text-xs">${registry ? `<a href="${htmlSafe(registry)}" target="_blank" rel="noopener" class="font-bold text-blue-600 hover:underline">Open</a>` : '<span class="text-slate-300">-</span>'}</td>
                <td class="py-3 px-3">
                    ${canEdit ? `<div class="flex items-center gap-2">
                        <button type="button" onclick="openCertificationStandardEditor('${htmlSafe(record.matrixType || '')}', '${rid}', false)" class="text-xs font-black text-purple-700 hover:underline">Edit</button>
                        <button type="button" onclick="deleteCertificationStandard('${rid}')" class="text-xs font-black text-red-500 hover:underline">Delete</button>
                    </div>` : '<span class="text-xs font-bold text-slate-300">View</span>'}
                </td>
            </tr>`;
        }
        function certificationStandardsTableHtml(records = [], canEdit = false) {
            if (!records.length) return '<div class="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-400">No standards in this group.</div>';
            return `<div class="overflow-x-auto rounded-2xl border border-cyan-200">
                <table class="w-full min-w-[1180px] text-left whitespace-nowrap">
                    <thead class="bg-teal-700 text-white">
                        <tr class="text-[10px] font-black uppercase tracking-widest">
                            <th class="py-3 px-3">Record ID</th>
                            <th class="py-3 px-3">Product Category</th>
                            <th class="py-3 px-3">Standard / Requirement</th>
                            <th class="py-3 px-3">Requirement Level</th>
                            <th class="py-3 px-3">Applicability Condition</th>
                            <th class="py-3 px-3">Evidence Type</th>
                            <th class="py-3 px-3">Project Applicability</th>
                            <th class="py-3 px-3">Registry URL</th>
                            <th class="py-3 px-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody>${records.map(record => certificationStandardRowHtml(record, canEdit)).join('')}</tbody>
                </table>
            </div>`;
        }
        function renderCertificationClassChecklist(profile) {
            const box = document.getElementById('cert-class-checklist');
            const summary = document.getElementById('cert-class-checklist-summary');
            const addBtn = document.getElementById('cert-class-add-standard-btn');
            if (!box) return;
            const canEdit = canEditCertificationStandards();
            if (addBtn) addBtn.classList.toggle('hidden', !canEdit);
            renderCertificationClassLevelFilters();
            const allRows = recordsForCertificationClass(profile);
            const selectedLevels = new Set(getCertificationClassLevelFilters());
            const rows = allRows.filter(record => selectedLevels.has(record.requirementLevel || 'Optional'));
            if (summary) summary.textContent = `${rows.length} visible / ${allRows.length} requirements`;
            const order = ['PV Module', 'Inverter / PCS', 'Battery / BESS', 'BOS', 'Other'];
            const groups = order.map(label => {
                const records = rows.filter(record => {
                    const group = certificationMatrixGroupLabel(record.matrixType);
                    return label === 'Other' ? !order.slice(0, -1).includes(group) : group === label;
                });
                return { label, records };
            }).filter(group => group.records.length);
            box.innerHTML = groups.length ? groups.map(group => {
                const counts = CERTIFICATION_LEVELS.map(level => `${group.records.filter(record => record.requirementLevel === level).length} ${level}`).join(' / ');
                return `<details class="rounded-2xl border border-slate-200 bg-white overflow-hidden">
                    <summary class="cursor-pointer select-none px-4 py-3 bg-slate-50 flex flex-wrap items-center justify-between gap-3">
                        <span class="text-sm font-black text-slate-800">${htmlSafe(group.label)}</span>
                        <span class="text-[11px] font-black text-slate-400">${group.records.length} items · ${htmlSafe(counts)}</span>
                    </summary>
                    <div class="p-3">${certificationStandardsTableHtml(group.records, canEdit)}</div>
                </details>`;
            }).join('') : '<div class="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-400">No requirements assigned to this class.</div>';
        }
        function updateCertificationClassTabs(activeId) {
            certificationClassProfiles.forEach(profile => {
                const btn = document.getElementById(`cert-class-tab-${profile.id}`);
                if (!btn) return;
                const active = profile.id === activeId;
                btn.className = active
                    ? 'px-3 py-2 rounded-xl border border-purple-200 bg-white text-xs font-black text-purple-800 shadow-sm'
                    : 'px-3 py-2 rounded-xl border border-slate-200 bg-white text-xs font-black text-slate-500 hover:bg-slate-50';
            });
        }
        function renderCertificationClassWorkspace() {
            ensureCertificationV4Data();
            const profile = getCertificationClassProfile(getSelectedCertificationClassId());
            updateCertificationClassTabs(profile.id);
            const condition = document.getElementById('cert-class-condition-card');
            if (condition) condition.innerHTML = certificationClassConditionHtml(profile);
            renderCertificationClassChecklist(profile);
        }
        function certificationLevelKey(level = '') {
            const text = String(level || '').trim().toLowerCase();
            if (text === 'mandatory') return 'mandatoryStandards';
            if (text === 'utility preferred') return 'preferredStandards';
            if (text === 'international finance preferred') return 'internationalFinanceStandards';
            return 'optionalStandards';
        }
        function certificationStandardExistingOptions(selectedId = '') {
            const selected = String(selectedId || '').trim();
            const options = normalizeCertificationStandardsCatalog(certificationStandardsCatalog)
                .sort((a, b) => String(a.matrixRecordId || '').localeCompare(String(b.matrixRecordId || ''), undefined, { numeric: true }))
                .map(record => `<option value="${htmlSafe(record.matrixRecordId)}" ${record.matrixRecordId === selected ? 'selected' : ''}>${htmlSafe(record.matrixRecordId)} · ${htmlSafe(record.standardRequirement || '')}</option>`)
                .join('');
            return `<option value="">Create new standard</option>${options}`;
        }
        function fillCertificationStandardEditor(record = {}) {
            const set = (id, value) => { const el = document.getElementById(id); if (el) el.value = value || ''; };
            set('cert-standard-record-id', record.matrixRecordId || '');
            set('cert-standard-matrix-type', record.matrixType || 'PV Module');
            set('cert-standard-product-category', record.productCategory || '');
            set('cert-standard-standard-requirement', record.standardRequirement || '');
            set('cert-standard-requirement-level', record.requirementLevel || 'Optional');
            set('cert-standard-applicability-condition', record.applicabilityCondition || '');
            set('cert-standard-evidence-type', record.evidenceType || '');
            set('cert-standard-project-applicability', record.projectApplicability || '');
            set('cert-standard-registry-url', record.officialRegistrySourceUrl || '');
            set('cert-standard-remarks', record.remarks || '');
        }
        function onCertificationStandardExistingRecordChange(matrixRecordId = '') {
            const record = getCertificationStandard(matrixRecordId);
            if (record) fillCertificationStandardEditor(record);
        }
        function nextCertificationMatrixRecordId(matrixType = '') {
            const type = String(matrixType || '').trim();
            const prefix = type === 'Inverter / PCS' ? 'INV' : type === 'Battery / BESS' ? 'BESS' : type === 'BOS' ? 'BOS' : 'PV';
            const used = normalizeCertificationStandardsCatalog(certificationStandardsCatalog)
                .map(record => String(record.matrixRecordId || ''))
        ...
        function openCertificationStandardEditor(matrixType = '', matrixRecordId = '', viewOnly = false) {
            ensureCertificationV4Data();
            const modal = document.getElementById('certification-standard-modal');
            if (!modal) return;
            const canEdit = canEditCertificationStandards() && !viewOnly;
            const record = getCertificationStandard(matrixRecordId) || normalizeCertificationStandard({
                matrixRecordId: '',
                matrixType: matrixType || 'PV Module',
                productCategory: matrixType || 'PV Module',
                requirementLevel: 'Mandatory',
                evidenceType: 'Certificate / Report',
                projectApplicability: 'All'
            });
            const existing = document.getElementById('cert-standard-existing-id');
            if (existing) existing.innerHTML = certificationStandardExistingOptions(record?.matrixRecordId || '');
            document.getElementById('cert-standard-original-id').value = record?.matrixRecordId || '';
            document.getElementById('cert-standard-editor-mode').value = 'class';
            document.getElementById('cert-standard-modal-title').textContent = record?.matrixRecordId ? (canEdit ? 'Edit Standard' : 'View Standard') : 'Add Standard';
            document.getElementById('cert-standard-modal-subtitle').textContent = 'Save will update the central certification standards catalog and add this Record ID to the selected Class requirements.';
            fillCertificationStandardEditor(record || {});
            if (!record?.matrixRecordId) {
                const type = matrixType || 'PV Module';
                document.getElementById('cert-standard-record-id').value = nextCertificationMatrixRecordId(type);
                document.getElementById('cert-standard-matrix-type').value = type;
            }
            modal.querySelectorAll('input, select, textarea').forEach(el => {
                if (el.id === 'cert-standard-existing-id' || el.type === 'hidden') return;
                el.disabled = !canEdit;
            });
            document.getElementById('cert-standard-save-btn')?.classList.toggle('hidden', !canEdit);
            document.getElementById('cert-standard-delete-btn')?.classList.toggle('hidden', !canEdit || !record?.matrixRecordId);
            modal.classList.remove('hidden');
        }
        function closeCertificationStandardEditor() {
            document.getElementById('certification-standard-modal')?.classList.add('hidden');
        }
        function readCertificationStandardFromEditor() {
            const get = id => String(document.getElementById(id)?.value || '').trim();
            return normalizeCertificationStandard({
                matrixRecordId: get('cert-standard-record-id'),
                matrixType: get('cert-standard-matrix-type'),
                productCategory: get('cert-standard-product-category'),
                standardRequirement: get('cert-standard-standard-requirement'),
                requirementLevel: get('cert-standard-requirement-level'),
                applicabilityCondition: get('cert-standard-applicability-condition'),
                evidenceType: get('cert-standard-evidence-type'),
                projectApplicability: get('cert-standard-project-applicability'),
                officialRegistrySourceUrl: get('cert-standard-registry-url'),
                remarks: get('cert-standard-remarks')
            });
        }
        function addCertificationStandardToCurrentClass(record) {
            const profile = getCertificationClassProfile(getSelectedCertificationClassId());
            const key = certificationLevelKey(record.requirementLevel);
            const token = record.matrixRecordId || record.standardRequirement;
            const next = { ...profile };
            next[key] = uniqueCertList([...(next[key] || []), token]);
            next.levelOverrides = { ...(next.levelOverrides || {}), [record.matrixRecordId]: record.requirementLevel };
            if (record.matrixType === 'PV Module') next.productGroups = uniqueCertList([...(next.productGroups || []), 'PV Module']);
            if (record.matrixType === 'Inverter / PCS') next.productGroups = uniqueCertList([...(next.productGroups || []), 'Inverter / PCS']);
            if (record.matrixType === 'Battery / BESS') next.productGroups = uniqueCertList([...(next.productGroups || []), 'Battery', 'ESS']);
            const idx = certificationClassProfiles.findIndex(item => item.id === next.id);
            if (idx >= 0) certificationClassProfiles[idx] = normalizeCertificationClassProfile(next);
            persistEntityToD1('certification_class_profile', next.id, next);
        }
        function saveCertificationStandardFromEditor() {
            if (!canEditCertificationStandards()) return;
            const record = readCertificationStandardFromEditor();
            if (!record?.matrixRecordId || !record.standardRequirement) {
                alert('Record ID and Standard / Requirement are required.');
                return;
            }
            const originalId = String(document.getElementById('cert-standard-original-id')?.value || '').trim();
            const duplicate = certificationStandardsCatalog.some(item => item.matrixRecordId === record.matrixRecordId && item.matrixRecordId !== originalId);
            if (duplicate) {
                alert(`Record ID ${record.matrixRecordId} already exists.`);
                return;
            }
            certificationStandardsCatalog = certificationStandardsCatalog.filter(item => item.matrixRecordId !== (originalId || record.matrixRecordId));
            certificationStandardsCatalog.push(record);
            addCertificationStandardToCurrentClass(record);
            persistEntityToD1('certification_standard', record.matrixRecordId, record);
            saveToLocal();
            closeCertificationStandardEditor();
            renderCertificationClassWorkspace();
        }
        function deleteCertificationStandardFromEditor() {
            const id = String(document.getElementById('cert-standard-original-id')?.value || '').trim();
            if (id) deleteCertificationStandard(id);
        }
        function deleteCertificationStandard(matrixRecordId = '') {
            if (!canEditCertificationStandards()) return;
            const id = String(matrixRecordId || '').trim();
            if (!id) return;
            const used = productCertificationEvidence.some(item => item.matrixRecordId === id);
            if (!confirm(used ? `Delete ${id}? Existing product evidence will keep a historical reference.` : `Delete ${id}?`)) return;
            certificationStandardsCatalog = certificationStandardsCatalog.filter(item => item.matrixRecordId !== id);
            certificationClassProfiles = certificationClassProfiles.map(profile => {
                const remove = list => (list || []).filter(token => token !== id);
                const levelOverrides = { ...(profile.levelOverrides || {}) };
                delete levelOverrides[id];
                return normalizeCertificationClassProfile({
                    ...profile,
                    mandatoryStandards: remove(profile.mandatoryStandards),
                    preferredStandards: remove(profile.preferredStandards),
                    internationalFinanceStandards: remove(profile.internationalFinanceStandards),
                    optionalStandards: remove(profile.optionalStandards),
                    levelOverrides
                });
            }).filter(Boolean);
            deleteEntityFromD1('certification_standard', id);
            saveToLocal();
            closeCertificationStandardEditor();
            renderCertificationClassWorkspace();
        }
        window.openCertificationStandardEditor = openCertificationStandardEditor;
        window.closeCertificationStandardEditor = closeCertificationStandardEditor;
        window.onCertificationStandardExistingRecordChange = onCertificationStandardExistingRecordChange;
        window.saveCertificationStandardFromEditor = saveCertificationStandardFromEditor;
        window.deleteCertificationStandardFromEditor = deleteCertificationStandardFromEditor;
        window.deleteCertificationStandard = deleteCertificationStandard;
        window.renderCertificationClassWorkspace = renderCertificationClassWorkspace;
        window.renderDb = () => {
        ...
        function getProductCertificationMatrixPriceListMeta(product) {
            ensureCertificationV4Data();
            const evidence = getProductCertificationEvidence(product?.id || '');
            const rows = getCertificationMatrixRowsForProduct(product || {});
            ...
            evidence.forEach(record => {
                if (record.projectApplicability) projectSet.add(record.projectApplicability);
                if (record.standardRequirement) standardSet.add(record.standardRequirement);
            });
            rows.forEach(record => {
                if (record.projectApplicability) projectSet.add(record.projectApplicability);
                if (record.standardRequirement) standardSet.add(record.standardRequirement);
            });
            const legacy = getProductCertificationRequirements(product || {});
            (legacy.standards || []).forEach(item => standardSet.add(item));
            return {
                projects: Array.from(projectSet).filter(Boolean),
                standards: Array.from(standardSet).filter(Boolean)
            };
        }
        function renderPriceListFilters() {
            ...
            products.forEach(p => {
                const meta = getProductCertificationMatrixPriceListMeta(p);
                meta.projects.forEach(project => projectOptions.add(project));
                const project = keep.country;
                if (project && !meta.projects.includes(project)) return;
                meta.standards.forEach(s => certSet.add(s));
            });
            const projects = Array.from(projectOptions).sort((a, b) => a.localeCompare(b));
            ...
            countrySel.innerHTML = `<option value="">All Project Applicability</option>` + projects.map(v => `<option value="${htmlSafe(v)}">${htmlSafe(v)}</option>`).join('');
            ...
            if (keep.country && projects.includes(keep.country)) countrySel.value = keep.country;
        ...
        function getFilteredPriceListProducts() {
            ...
            const project = String(document.getElementById('price-list-country-filter')?.value || '').trim();
            const cert = String(document.getElementById('price-list-cert-filter')?.value || '').trim();
            return products.filter(p => {
                const meta = getProductCertificationMatrixPriceListMeta(p);
                const text = `${p.id || ''} ${p.name || ''} ${p.spec || ''} ${p.category || ''} ${p.scenario || ''} ${getProductSupplierDisplay(p)} ${meta.standards.join(' ')} ${meta.projects.join(' ')}`.toLowerCase();
                if (q && !text.includes(q)) return false;
                ...
                if (project && !meta.projects.includes(project)) return false;
                if (cert && !meta.standards.includes(cert)) return false;
                return true;
        ...
        window.renderPriceList = () => {
            ...
            body.innerHTML = rows.map(p => {
                const certMeta = getProductCertificationMatrixPriceListMeta(p);
                const pricing = priceListProductPricing(p);
                ...
                const projects = certMeta.projects.slice(0, 2).join(', ');
                const certBrief = certMeta.standards.slice(0, 3).join(', ');
                const certTitle = `${certMeta.projects.join(', ')}\n${certMeta.standards.join('\n')}`;
                ...
                return `
                    <tr class="hover:bg-purple-50/40 transition-colors" onmousemove="showPriceListTooltip(event, '${htmlSafe(p.id)}')" onmouseleave="hidePriceListTooltip()">
                        ...
                        <td class="py-4 px-4 text-xs text-slate-500 max-w-[220px] truncate" title="${htmlSafe(certTitle)}">${htmlSafe(certBrief || '-')}<div class="text-[10px] text-slate-400">${htmlSafe(projects || '-')}</div></td>
                    </tr>
                `;
            }).join('');
            updatePriceListSelectionUi();
        ...
        window.exportSelectedPriceListExcel = () => {
            ...
            const rows = selected.map(p => {
                ...
                const out = {
                    ...
                    'Project Applicability': certMeta.projects.join('; '),
                    'Certification Standards': certMeta.standards.join('; ')
                };
                return out;
            });
        ...
        window.openModal = () => {
            ...
            if (!window.editId) {
                ...
                window.__productImageDraft = '';
                renderProductImagePreview();
                renderProductCertificationMatrixSummary({ id: '', name: '', category: document.getElementById('m-category')?.value || '' });
            }
        ...
        window.closeModal = () => {
            ...
            window.editId = null;
            ['m-name', 'm-category', 'm-supplier-code', 'm-spec', 'm-inverter-kw', 'm-battery-kwh', 'm-scenario', 'm-warranty-years', 'm-warranty-cycles', 'm-lead-time', 'm-contact', 'm-contact-info', 'm-price-basis-unit', 'm-unit-qty-per-pcs', 'm-cost', 'm-price', 'm-source-type', 'm-commercial-supplier-code', 'm-factory-supplier-code', 'm-brand-owner-supplier-code', 'm-authorization-status', 'm-authorization-expiry', 'm-source-remark', ...PRODUCT_MASTER_COMMON_FIELD_KEYS.map(key => `m-master-${key}`)].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.value = '';
            });
            ...
            updateProductPriceCurrencyUi();
            const summary = document.getElementById('m-cert-matrix-summary');
            if (summary) summary.textContent = 'No certification evidence selected yet.';
            window.__productImageDraft = '';
            const fileEl = document.getElementById('m-product-image-file');
            if (fileEl) fileEl.value = '';
            renderProductImagePreview();
        ...
        window.saveProduct = () => {
            ...
            if (window.editId) {
                ...
                if (existing?.certificationRequirements) {
                    data.certificationRequirements = existing.certificationRequirements;
                }
            }
        ...
        window.editProduct = (id) => {
            ...
            updateProductPriceCurrencyUi();
            renderProductCertificationMatrixSummary(p);
        ...
        let importCertificationStandardsData = [];
        let importCertificationEvidenceData = [];
        const CERTIFICATION_MATRIX_HEADERS = [
            'Record ID',
            'Product ID',
            'Manufacturer',
            'Brand',
            ...
            'BOM Coverage Status',
            'BOM Revision / Critical Components',
            'Report Scope / Key Result',
            'Verification Status',
            'Verified By',
        ...
        const CERTIFICATION_MATRIX_HEADER_KEYS = {
            'Record ID': 'matrixRecordId',
            ...
            'Report No.': 'reportNo',
            'Certification / Listing Body': 'certificationListingBody',
            'Testing Laboratory': 'testingLaboratory',
            'Laboratory Accreditation / Scope': 'laboratoryAccreditationScope',
            'Standard Edition / Version': 'standardEditionVersion',
            'Issue Date': 'issueDate',
            'Expiry Date': 'expiryDate',
            'Days to Expiry': 'daysToExpiry',
            'Expiry Status': 'expiryStatus',
            'Model Coverage Status': 'modelCoverageStatus',
            'Model Coverage Details': 'modelCoverageDetails',
            'Factory Coverage Status': 'factoryCoverageStatus',
            'Factory / Production Site': 'factoryProductionSite',
            'BOM Coverage Status': 'bomCoverageStatus',
            'BOM Revision / Critical Components': 'bomRevisionCriticalComponents',
            'Report Scope / Key Result': 'reportScopeKeyResult',
            'Verification Status': 'verificationStatus',
            'Verified By': 'verifiedBy',
            'Last Validation Date': 'lastValidationDate',
            'Next Review Date': 'nextReviewDate',
            'Official Registry Verified': 'officialRegistryVerified',
            'Official / Registry Source URL': 'officialRegistrySourceUrl',
            'Document Link': 'documentLink',
            'Project Applicability': 'projectApplicability',
            'Linked Project ID': 'linkedProjectId',
            'Deviation / NCR': 'deviationNcr',
            'Remarks': 'remarks',
            'Auto Completeness Check': 'autoCompletenessCheck'
        };
        const systemFields = {
            id: '产品编号',
            name: '产品全称',
        ...
        const systemFieldLabelsEnglish = {
            ...
            certificationCountries: 'Legacy Certification Markets',
        ...
        window.closeImportModal = () => {
            ...
            importCertificationStandardsData = [];
        ...
        function sheetRowsByDetectedHeader(worksheet, requiredHeader = 'Record ID') {
            if (!worksheet) return [];
            const rows = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '' });
            const headerIndex = rows.findIndex(row => row.map(cell => String(cell || '').trim()).includes(requiredHeader));
            if (headerIndex < 0) return XLSX.utils.sheet_to_json(worksheet, { defval: '' });
        ...
        function certificationMatrixObjectFromRow(row, opts = {}) {
            const out = {};
            CERTIFICATION_MATRIX_HEADERS.forEach(header => {
                const key = CERTIFICATION_MATRIX_HEADER_KEYS[header];
                if (!key) return;
                out[key] = row?.[header] ?? '';
            });
            out.matrixRecordId = out.matrixRecordId || row?.['Standard Code'] || row?.Code || row?.code || '';
            out.standardRequirement = out.standardRequirement || row?.Name || row?.name || row?.['Standard / Requirement'] || '';
            out.requirementLevel = out.requirementLevel || row?.Level || row?.level || '';
            out.evidenceType = out.evidenceType || row?.['Document Type'] || row?.['Evidence Type'] || row?.documentType || '';
            out.applicabilityCondition = out.applicabilityCondition || row?.['Applicability Condition'] || '';
            out.projectApplicability = out.projectApplicability || row?.['Project Applicability'] || '';
            out.officialRegistrySourceUrl = out.officialRegistrySourceUrl || row?.['Official / Registry Source URL'] || '';
            out.remarks = out.remarks || row?.Remarks || row?.Remark || '';
            if (opts.matrixSheet) out.matrixSheet = opts.matrixSheet;
            if (opts.matrixType) out.matrixType = opts.matrixType;
            return out;
        }
        ...
        function parseExcel(file) {
            ...
            reader.onload = (e) => {
                ...
                const evidenceSheet = workbook.Sheets['Certification Evidence'];
                const standardsSheet = workbook.Sheets['Certification Standards'];
                if (standardsSheet) {
                    const standardRows = sheetRowsByDetectedHeader(standardsSheet, 'Record ID');
                    importCertificationStandardsData = standardRows
                        .map(row => normalizeCertificationStandard(certificationMatrixObjectFromRow(row)))
                        .filter(record => record.matrixRecordId && record.standardRequirement);
                }
                if (evidenceSheet) {
                    const evidenceRows = sheetRowsByDetectedHeader(evidenceSheet, 'Record ID');
                    importCertificationEvidenceData = evidenceRows
                        .map(row => normalizeProductCertificationEvidence(certificationMatrixObjectFromRow(row)))
                        .filter(record => record.productId && record.matrixRecordId);
                }
        ...
        function renderFieldMapping() {
            ...
            const aliases = {
                ...
                certificationCountries: ['产品认证国家', '认证国家', 'Legacy Certification Markets'],
        ...
        function processImport() {
            ...
            importData.forEach((row, i) => {
                ...
                if (newProduct.certificationCountries || newProduct.certificationStandards) {
                    ...
                    standards.forEach(standardText => {
                        const standard = getCertificationStandard(standardText)
                            || certificationStandardsCatalog.find(item => String(item.standardRequirement || '').toLowerCase() === String(standardText || '').toLowerCase())
                            || certificationStandardsCatalog.find(item => String(item.standardRequirement || '').toLowerCase().includes(String(standardText || '').toLowerCase()));
                        if (!standard?.matrixRecordId) return;
                        const evidence = normalizeProductCertificationEvidence({
                            productId: newProduct.id,
                            matrixRecordId: standard.matrixRecordId,
                            standardRequirement: standard.standardRequirement,
                            requirementLevel: standard.requirementLevel,
                            evidenceType: standard.evidenceType,
                            isApplicable: standard.isApplicable || 'Conditional',
                            evidenceAvailable: 'No',
                            verificationStatus: 'Not Reviewed',
                            projectApplicability: standard.projectApplicability,
                            remarks: `Imported from legacy Product List certification columns${countries.length ? `; legacy markets: ${countries.join(', ')}` : ''}`
                        });
                        if (evidence) importCertificationEvidenceData.push(evidence);
                    });
            ...
            const certificationStandardCount = processCertificationStandardsImport();
            const certificationEvidenceCount = processCertificationEvidenceImport();
            ...
            try {
                window.__minovaBusiness?.upsertEntities?.([
                    ...
                    ...certificationStandardsCatalog.map(record => ({ domain: 'certification_standard', recordId: record.matrixRecordId || record.code, payload: cloneForD1(record) })),
                    ...certificationClassProfiles.map(record => ({ domain: 'certification_class_profile', recordId: record.id, payload: cloneForD1(record) })),
                    ...productCertificationEvidence.map(record => ({ domain: 'certification_evidence', recordId: record.id, payload: cloneForD1(record) }))
                ]);
            } catch (e) {
            ...
            if (certificationStandardCount) log.push(`<p class="text-green-600">Certification Standards imported: ${certificationStandardCount} records.</p>`);
            if (certificationEvidenceCount) log.push(`<p class="text-green-600">Certification Evidence imported: ${certificationEvidenceCount} records.</p>`);
        ...
        function processCertificationStandardsImport() {
            if (!Array.isArray(importCertificationStandardsData) || !importCertificationStandardsData.length) return 0;
            let count = 0;
            const map = new Map(normalizeCertificationStandardsCatalog(certificationStandardsCatalog).map(record => [record.matrixRecordId || record.code, record]));
            importCertificationStandardsData.forEach(record => {
                const standard = normalizeCertificationStandard(record);
                const key = standard?.matrixRecordId || standard?.code || '';
                if (!key) return;
                map.set(key, standard);
                count += 1;
            });
            certificationStandardsCatalog = Array.from(map.values());
            return count;
        }

        function processCertificationEvidenceImport() {
            if (!Array.isArray(importCertificationEvidenceData) || !importCertificationEvidenceData.length) return 0;
            let count = 0;
            importCertificationEvidenceData.forEach(record => {
                const evidence = normalizeProductCertificationEvidence(record);
                if (!evidence || !evidence.productId || !evidence.matrixRecordId) return;
                const idx = productCertificationEvidence.findIndex(item => item.id === evidence.id);
                if (idx >= 0) productCertificationEvidence[idx] = evidence;
                else productCertificationEvidence.push(evidence);
                count += 1;
            });
            productCertificationEvidence = normalizeProductCertificationEvidenceList(productCertificationEvidence);
            return count;
        ...
        window.downloadTemplate = () => {
            ...
            const headers = ['Product ID', 'Product Name', 'Category', 'Supplier', 'Spec Model', 'Inverter kW', 'Battery kWh', 'Subcategory', 'Warranty Years', 'Cycle Count', 'Lead Time', 'Contact', 'Contact Method', 'Legacy Certification Standards', 'Base Currency', 'Price Basis Unit', 'Qty per PCS', 'Base Cost', 'Base Price', ...masterHeaders, ...sourcingHeaders, ...technicalHeaders];
            ...
            // 如果没数据，加一行示例
            if (data.length === 0) {
                data.push(['PROD001', 'Sample All-in-One', 'All-in-One System', 'Generic Supplier', '5.5 kW / 10 kWh', 5.5, 10, 'Single-Phase All-in-One', '10', '0', '15 days', 'Sales Manager', 'sales@example.com', 'IEC 62619; IEC 62109-1/2', 'CNY', 'set', 1, 5000, 6500, 'AIO-5.5-10', 'Generic Brand', 'Hybrid Series', 'Residential', 'LV', 'Single', 'Active', 'MY', '', '', 'Sample V3 Product Master row', 'Direct Factory', '', 'SUP001', 'SUP001', 'SUP001', 'SUP001', 'Authorized', '', 'Factory source confirmed', ...PRODUCT_MASTER_TECHNICAL_IMPORT_KEYS.map(key => key === 'nominalEnergyKwh' ? 10 : (key === 'pcsRatedPowerKw' ? 5.5 : ''))]);
            }
            ...
            ensureCertificationV4Data();
            const certificationRowForExport = (record, opts = {}) => {
                const standard = record?.matrixRecordId ? getCertificationStandard(record.matrixRecordId) : null;
                const source = { ...(standard || {}), ...(record || {}) };
                const out = {
                    'Record ID': source.matrixRecordId || source.code || '',
                    'Product ID': opts.includeProductId ? (source.productId || '') : '',
                    'Manufacturer': source.manufacturer || '',
                    'Brand': source.brand || '',
                    'Product Category': source.productCategory || source.productType || '',
                    'Product Model': source.productModel || '',
                    'Standard / Requirement': source.standardRequirement || source.name || '',
                    'Requirement Level': source.requirementLevel || source.level || '',
                    'Applicability Condition': source.applicabilityCondition || '',
                    'Evidence Type': source.evidenceType || source.documentType || '',
                    'Is Applicable': source.isApplicable || '',
                    'Evidence Available': source.evidenceAvailable || '',
                    'Certificate No.': source.certificateNo || '',
                    'Report No.': source.reportNo || '',
                    'Certification / Listing Body': source.certificationListingBody || '',
                    'Testing Laboratory': source.testingLaboratory || '',
                    'Laboratory Accreditation / Scope': source.laboratoryAccreditationScope || '',
                    'Standard Edition / Version': source.standardEditionVersion || '',
                    'Issue Date': source.issueDate || '',
                    'Expiry Date': source.expiryDate || '',
                    'Days to Expiry': source.daysToExpiry || '',
                    'Expiry Status': source.expiryStatus || '',
                    'Model Coverage Status': source.modelCoverageStatus || '',
                    'Model Coverage Details': source.modelCoverageDetails || '',
                    'Factory Coverage Status': source.factoryCoverageStatus || '',
                    'Factory / Production Site': source.factoryProductionSite || '',
                    'BOM Coverage Status': source.bomCoverageStatus || '',
                    'BOM Revision / Critical Components': source.bomRevisionCriticalComponents || '',
                    'Report Scope / Key Result': source.reportScopeKeyResult || '',
                    'Verification Status': source.verificationStatus || '',
                    'Verified By': source.verifiedBy || '',
                    'Last Validation Date': source.lastValidationDate || '',
                    'Next Review Date': source.nextReviewDate || '',
                    'Official Registry Verified': source.officialRegistryVerified || '',
                    'Official / Registry Source URL': source.officialRegistrySourceUrl || '',
                    'Document Link': source.documentLink || '',
                    'Project Applicability': source.projectApplicability || '',
                    'Linked Project ID': source.linkedProjectId || '',
                    'Deviation / NCR': source.deviationNcr || '',
                    'Remarks': source.remarks || source.remark || '',
                    'Auto Completeness Check': source.autoCompletenessCheck || ''
                };
                return CERTIFICATION_MATRIX_HEADERS.map(header => out[header] ?? '');
            };
            const standardsHeaders = CERTIFICATION_MATRIX_HEADERS;
            const standardsRows = certificationStandardsCatalog.map(record => certificationRowForExport(record));
            const standardsWorksheet = XLSX.utils.aoa_to_sheet([standardsHeaders, ...standardsRows]);
            const evidenceHeaders = CERTIFICATION_MATRIX_HEADERS;
            const evidenceRows = productCertificationEvidence.map(record => certificationRowForExport(record, { includeProductId: true }));
            if (!evidenceRows.length) evidenceRows.push(certificationRowForExport({
                productId: 'PROD001',
                ...
                verificationStatus: 'Not Reviewed',
                documentLink: '',
                projectApplicability: 'Grid-connected only',
                remarks: 'Sample certification evidence row'
            }, { includeProductId: true }));
            const evidenceWorksheet = XLSX.utils.aoa_to_sheet([evidenceHeaders, ...evidenceRows]);
            const workbook = XLSX.utils.book_new();

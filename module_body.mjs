        ...
        const initGitHubSync = (() => {
            ...
            function createRepoStore({ api }) {
                ...
                return { upsertText, commitTextFiles };
            }
            ...
            function mountGitHubSyncUi({ sync }) {
                ...
                btnPublish.onclick = async () => {
                    ...
                    try {
                        ...
                        startDots('已提交，请稍等');
                        const expectedAt = (() => {
                            const m = html.match(/<script id="minova-embedded-state" type="application\/json">([\s\S]*?)<\/script>/);
                            if (!m?.[1]) return 0;
                        ...
                        const fetchDeployedAt = async () => {
                            ...
                            const text = await res.text();
                            const m = text.match(/<script id="minova-embedded-state" type="application\/json">([\s\S]*?)<\/script>/);
                            if (!m?.[1]) return 0;
                            try { return Date.parse(JSON.parse(m[1]).updatedAt || '') || 0; } catch { return 0; }
                        };
...
`;
        ...
        function applyStateFromData(data, stampMs = 0) {
            ...
            savedQuotes = Array.isArray(data?.savedQuotes) ? data.savedQuotes : [];
            try {
        ...
        var quoteRows = [{ id: Date.now(), description: '', vendor: '', spec: '', batchNo: '', quantity: 1, price: 0, cost: 0, productId: '', inventoryId: '' }];
        var dbGroupMode = 'category';
        var currentLang = 'zh';
        var currentCurrency = 'CNY';
        var paymentTermsConfirmed = true;
        ...
        function extractStateJsonFromHtml(html) {
            const m = String(html || '').match(/<script id="minova-embedded-state"[^>]*>([\s\S]*?)<\/script>/i);
        ...
        window.updateCertSectionCount = (section) => {
            ...
            if (!countEl) return;
            const checkboxes = modal.querySelectorAll(`#qa-${section}-cert-body .cert-checkbox`);
            const checked = modal.querySelectorAll(`#qa-${section}-cert-body .cert-checkbox:checked`);
        ...
        window.confirmAndGeneratePDF = () => {
            ...
            const checkboxes = document.querySelectorAll('#pdf-page-checkboxes input[type="checkbox"]:checked');
            const selectedPages = Array.from(checkboxes).map(cb => parseInt(cb.value));
            
            if (selectedPages.length === 0) {
                selectedPages.push(1); // Default to page 1
            }
            ...
            selectedPages.forEach((pageNum, index) => {
                const pageEl = document.getElementById(`quote-page-${pageNum}`);
                if (pageEl) {
                    const printContainer = pageEl.querySelector('.print-container');
                    if (printContainer) {
                        const clone = printContainer.cloneNode(true);
                        
                        // Copy inputs
                        const originalInputs = printContainer.querySelectorAll('input, textarea, select');
                            ...
                            if (clonedInputs[i]) {
                                if (orig.tagName === 'INPUT' || orig.tagName === 'TEXTAREA') {
                                    ...
                                    if (orig.type === 'checkbox' || orig.type === 'radio') {
                                        clonedInputs[i].checked = orig.checked;
                                    }
            ...
            const opt = {
                ...
                html2canvas: {
                    ...
                    onclone: (clonedDoc) => {
                        const conts = clonedDoc.querySelectorAll('.print-container');
                        ...
                        clonedDoc.querySelectorAll('textarea, input').forEach(el => {
                            ...
                            const classesToCopy = ['font-bold', 'text-slate-700', 'uppercase', 'tracking-tighter', 'font-black', 'text-slate-900', 'text-sm', 'text-slate-500', 'text-[', 'text-xs', 'w-'];
            ...
            const fetchAttachmentArrayBuffer = async (path) => {
                const cfg = window.__minovaSync?.config || {};
                const owner = cfg.owner;
                const repo = cfg.repo;
                const branch = cfg.branch || 'main';
            ...
            html2pdf().set(opt).from(element).toPdf().get('pdf').then(async (pdf) => {
                ...
                const qPages = await mergedDoc.copyPages(quoteDoc, quoteDoc.getPageIndices());
                ...
                for (const f of selectedFiles) {
                    ...
                    if (!path) continue;
                    try {
                        const ab = await fetchAttachmentArrayBuffer(path);
                        if (ext === 'pdf') {
                            const ad = await PDFDocument.load(ab);
                            const pages = await mergedDoc.copyPages(ad, ad.getPageIndices());
                ...
                if (skipped.length || failed.length) {
                    alert(`已生成合并 PDF。\n跳过（非 PDF/图片）：${skipped.length ? skipped.join('、') : '无'}\n下载失败：${failed.length ? failed.join('、') : '无'}`);
                }

            }).catch((e) => {
                alert('生成 PDF 失败：' + String(e?.message || e || ''));
            }).finally(() => {
...
function clearRefImage() {
    ...
    const ph = document.getElementById('ref-image-placeholder');
    if(ph) ph.classList.remove('hidden');
    const uploader = document.getElementById('ref-image-upload');
...
function clearSiteImage() {
    ...
    const uploader = document.getElementById('site-image-upload');
    if (uploader) uploader.value = '';
    
    const btnAdd = document.getElementById('btn-add-pv-panel');
    if (btnAdd) btnAdd.style.display = 'none';
    const btnClear = document.getElementById('btn-clear-site');
...
// Override updateLanguageLabels to call ours too
const originalUpdateLanguageLabels = window.updateLanguageLabels;
window.updateLanguageLabels = () => {
    if (originalUpdateLanguageLabels) originalUpdateLanguageLabels();
    updateMultiPageLanguageLabels();
};

// Override toggleCurrency to call calculateROI to update symbols
const originalToggleCurrency = window.toggleCurrency;
window.toggleCurrency = () => {
    if (originalToggleCurrency) originalToggleCurrency();
    calculateROI();
};

// EXPORT TO WINDOW (GLOBAL)
window.renderSavedQuotesList = renderSavedQuotesList;
window.switchQuoteSubpage = switchQuoteSubpage;
window.createNewQuote = createNewQuote;
window.fileToBase64 = fileToBase64;
window.resizeImageBase64 = resizeImageBase64;
window.calculateROI = calculateROI;
window.renderPartBreakdown = renderPartBreakdown;
window.uploadPartImage = uploadPartImage;
window.uploadRefImage = uploadRefImage;
window.clearRefImage = clearRefImage;
window.uploadSiteImage = uploadSiteImage;
window.clearSiteImage = clearSiteImage;
window.addSitePVPanel = addSitePVPanel;
window.renderSiteWorkspace = renderSiteWorkspace;
window.deletePVPanel = deletePVPanel;
window.saveCurrentQuote = saveCurrentQuote;
window.onSavedQuoteSelect = onSavedQuoteSelect;
window.loadSavedQuote = loadSavedQuote;
window.updateMultiPageLanguageLabels = updateMultiPageLanguageLabels;
